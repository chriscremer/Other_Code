...
if load_:
    mask = pickle.load( open( save_dir+'mask_'+str(load_val)+'.pkl', "rb" ) )[0]
    mask = Variable(torch.from_numpy(mask).cuda(), requires_grad=True)
    print ('loaded mask', save_dir+'mask_'+str(load_val)+'.pkl')
else:
    mask = Variable(torch.Tensor(1,2,84,84).normal_(0,.1).cuda(), requires_grad=True)
...
# optimizer = optim.Adam([mask], lr=.00005) #, weight_decay=.00001)
optimizer = optim.Adam([mask], lr=.0005) #, weight_decay=.00001)
# optimizer = optim.SGD([mask], lr=.05)#, momentum=0.1) #, weight_decay=.00001)
...
for step in range(steps):
    ...
    if step%500==0: # and batch_idx == 0:
        print ('Step: {}/{}'.format(step+1, steps),
            ...
            'mask_max:{:.4f}'.format(torch.max(mask_sig).data[0]),
            'mask_min:{:.4f}'.format(torch.min(mask_sig).data[0]),
            # 'std:{:.4f}'.format(torch.mean(torch.sqrt(torch.exp(mask))).data[0]),

            # 'minstd:{:.4f}'.format(torch.min(torch.sqrt(torch.exp(mask))).data[0]),
            # 'maxstd:{:.4f}'.format(torch.max(torch.sqrt(torch.exp(mask))).data[0]),

            # 'mask:{:.4f}'.format(torch.mean(mask).data[0]),
            # 'lr:{:.5f}'.format(lr),
            )
...
toplot =mask_sig.data.cpu().numpy()[0]
...
plt_path = save_dir+'viz_mask_'+str(traj_ind)+'_'+str(start_ind)+'_3.png'
