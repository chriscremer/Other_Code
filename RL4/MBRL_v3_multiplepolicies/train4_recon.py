...
from a2c_agents_original import a2c



from train_utils import do_vid, do_gifs, do_params, do_ls, update_ls_plot, save_params_v2, load_params_v2, save_params_v3
...
def train(model_dict):
    ...
    vae = VAE()
    load_epoch = 558
    path_to_load_variables = home+'/Documents/tmp/multiple_seeds_of_policies/vae_params'+str(load_epoch)+'_withpolicy.ckpt'
    ...
    for j in range(num_updates):
        for step in range(num_steps):
            ...
            #convert image
            # original_state = Variable(agent.rollouts.states[step]) / 255.
            original_state = agent.rollouts.states[step] / 255.

            recon_state = vae.reconstruction2(original_state)


            # print (original_state.size())
            # print (recon_state.size())
            ...
            value, action, action_log_probs, dist_entropy = agent.act(Variable(recon_state.data))#, volatile=True))
