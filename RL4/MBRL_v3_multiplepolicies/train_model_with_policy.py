...
if viz_:
    ...
    for j in range(len(to_plot)):
        ...
        for i in range(rows):
            ...
            ax.text(0.4, 1.04, 'Recon', transform=ax.transAxes, family='serif', size=6)
            ...
            #Plot the grad of the recon
            ax = plt.subplot2grid((rows,cols), (i,4), frameon=False)
            ...
            ax.text(0.4, 1.04, 'Grad of Recon', transform=ax.transAxes, family='serif', size=6)





            # #Plot the prediction of the true grad
            # ax = plt.subplot2grid((rows,cols), (i,5), frameon=False)

            # frame = dataset[traj_ind][start_ind+i][1]  #[2,84,84]
            # x = Variable(torch.from_numpy(np.array([frame])).float()).cuda()
            # true_grad_pred = vae.predict_grad(x)[0]  #[2,84,84]
            # # print (true_grad_pred)
            # true_grad_pred = np.abs(true_grad_pred.data.cpu().numpy())
            ...
            # state1 = np.concatenate([true_grad_pred[0], true_grad_pred[1]] , axis=1)
            # # ax.imshow(state1, cmap='gray', norm=NoNorm())
            # ax.imshow(state1, cmap='gray')
            # ax.set_xticks([])
            # ax.set_yticks([])

            # ax.text(0.4, 1.04, 'Pred Grad of Real', transform=ax.transAxes, family='serif', size=6)





            # #Plot the prediction of the recon grad
            # ax = plt.subplot2grid((rows,cols), (i,6), frameon=False)
            ...
            # ax.text(0.4, 1.04, 'Pred Grad of Recon', transform=ax.transAxes, family='serif', size=6)
