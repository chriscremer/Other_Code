...
class VAE(nn.Module):
    ...
    def forward(self, x, policies, k=1):
        ...
        grad_true = Variable(grad_true.view(self.B, -1).data) #[B,X]
        grad_true = torch.abs(grad_true)
        # print (grad_true.size())
        # print (torch.sum((grad_true > 0.).float())/(14112*40)) #/(grad_true.size()[1]))

        # fasd

        # print (torch.mean(grad_true))
        # fsdafd
        # median_val = torch.unsqueeze(torch.median(grad_true, dim=1)[0],1)  #[B,1]

        grad_true = grad_true / torch.max(grad_true, dim=1, keepdim=True)[0]

        median_val = torch.unsqueeze(torch.mean(grad_true, dim=1),1)  #[B,1]
        grad_true = grad_true - median_val # - .001
        grad_true = torch.clamp(grad_true, min=0)
        # print (torch.sum((grad_true > 0.).float())/(14112*40))

        median_val = torch.unsqueeze(torch.mean(grad_true, dim=1),1) 
        grad_true = grad_true - median_val # - .001
        grad_true = torch.clamp(grad_true, min=0)
        # print (torch.sum((grad_true > 0.).float())/(14112*40))
        ...
        grad_true = grad_true - (torch.max(grad_true, dim=1, keepdim=True)[0] - torch.min(grad_true, dim=1, keepdim=True)[0] )/2.
        grad_true = torch.clamp(grad_true, min=0)
        # print (torch.sum((grad_true > 0.).float())/(14112*40))
        # fasd

        grad_true = torch.clamp(grad_true, max=1)

        # fads


        # grad_true = grad_true - torch.min(grad_true, dim=1, keepdim=True)[0]
        # grad_true = grad_true / torch.max(grad_true, dim=1, keepdim=True)[0]
        ...
        # print (grad_true.size())  
        # fdsaf
        # print (torch.min(grad_true, dim=1)[0].size())
        # fas
        # fas
        # grad_true = grad_true / torch.max(grad_true)


        # print (torch.max(grad_true, dim=1)[0])
        # print (torch.min(grad_true, dim=1)[0])
        # print (torch.mean(grad_true, dim=1))
        # print (torch.median(grad_true, dim=1)[0])

        # fadfd

        # #See values 
        # grad_true = grad_true.data.cpu().numpy()[0]
        # print (grad_true.shape)
        # print (np.sort(grad_true)[:10])
        # print (np.sort(grad_true)[::-1][:10])
        # fads
        ...
        # print (logpx.size())
        # fdsf
        weighted_logpx = logpx * Variable(grad_true.data)
        ...
        loss =  - torch.mean(weighted_logpx) *100.
    ...
    def train(self, train_x, epochs, policies):
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (data, target) in enumerate(train_loader):
                ...
                loss, logpx = self.forward(batch, policies, k=k)
                # loss = -(elbo)
