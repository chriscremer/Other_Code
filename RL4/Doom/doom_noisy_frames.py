...
# The idea here is that ill do a weighted combination of the real frame and a blurred one
    # the blur can be mean blur or gaussian blur. 
    # multiple blur levels can used .
    #this is like discretized version of v1, but way faster and fewer params
    #this could possibly replace the mask
...
class BLUR_PREDICTOR(nn.Module):
    ...
    def forward(self, frame, DQNs):
        ...
        if torch.max(blurred_frame) > 1.:
            print ('ehyeheyy')
            print (torch.max(blurred_frame).data.cpu().numpy())
            fasdf

        # print (blurred_frame.shape)
        # print (torch.max(blurred_frame))
        # fsfa
        ...
        # blurred_frame = F.conv2d(frame, filter_weights, padding=padding, stride=1)
        # # print (blurred_frame.shape)
        # # fds
...
if __name__ == '__main__':
    ...
    if start_epoch>0:
        ...
        load_params_v3(save_dir=exp_path + 'blur_v2_moretrainin500/', model=MP, epochs=start_epoch)
    MP.cuda()
    # n_gpus = 2
    ...
    if viz_:
        ...
        while not game.is_episode_finished() and count<max_count:
            ...
            if count %4==0:
                ...
                n, p, l = 1, .5, 19
                r = 25
                bin_var = np.random.binomial(n, p, l)
                print (bin_var)


                
                bin_var = np.repeat(bin_var, r)
                print (bin_var.shape)

                bin_var = np.concatenate((bin_var, np.ones([640-r*l])), axis=0)
                print (bin_var.shape)


                vert_len = r*6
                # bin_var = np.broadcast_to(bin_var, (r,640))
                bin_var = np.broadcast_to(bin_var, (vert_len,640))


                # print (bin_var)                
                print (bin_var.shape) #  [r,r*l]              


                bin_var = np.concatenate((bin_var, np.ones([480-vert_len,640])), axis=0)
                print (bin_var.shape)

                bin_var = np.reshape(bin_var, [1,480,640])
                print (bin_var.shape)
                ...
                x = Variable(torch.from_numpy(np.array([frame])).float()).cuda()
                # print (x.size())  #[1,3,480,640]
                # fdsfas
                ...
                ax.set_xticks([])
                ax.set_yticks([])
                ax.text(0.4, 1.04, 'Mixed', transform=ax.transAxes, family='serif', size=6)
