...
class VAE(nn.Module):
    def __init__(self):
        ...
        self.intermediate_size = 1728 #1536

        #MASK PREDICTOR
        self.conv1_gp = nn.Conv2d(image_channels, 32, kernel_size=16, stride=8)
        self.conv2_gp = nn.Conv2d(32, 64, kernel_size=7, stride=4)
        self.conv3_gp = nn.Conv2d(64, 32, kernel_size=(4,3), stride=2)
        ...
        self.deconv1_gp = torch.nn.ConvTranspose2d(in_channels=32, out_channels=64, kernel_size=(4,3), stride=2)#, output_padding=(0,1))
        self.deconv2_gp = torch.nn.ConvTranspose2d(in_channels=64, out_channels=64, kernel_size=7, stride=4)#, output_padding=(3,3))
        self.deconv3_gp = torch.nn.ConvTranspose2d(in_channels=64, out_channels=3, kernel_size=16, stride=8)#, output_padding=(4,4))
    ...
    def reconstruct(self, x):
        ...
        # print (x.size())
        x = self.act_func(self.conv2_gp(x)) #[B,64,14,19]
        # print (x.size())
        x = self.act_func(self.conv3_gp(x)) #[B,32,6,9]
        # print (x.size())
        # print()
...
if __name__ == '__main__':
    ...
    exp_path_2 = exp_path + 'doom_vae_findbetterpadding/'
    ...
    start_epoch = 10 #270 #1000 # 570 #500
    ...
    if train_blur_net_:
        ...
        vae.train2(epochs=epochs, DQN=DQNs, memory=memory, game=game)
