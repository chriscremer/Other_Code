...
class MASK_PREDICTOR(nn.Module):
    ...
    def forward(self, frame, DQNs):
        ...
        masked_frame = frame * mask + (1.-mask)* F.sigmoid(self.bias_frame.bias_frame)
...
if __name__ == '__main__':
    ...
    exp_path_2 = exp_path + '2DQNs_biasframe/'
    ...
    if viz_:
        ...
        while not game.is_episode_finished() and count<max_count:
            ...
            if count %4==0:
                ...
                ax.set_xticks([])
                ax.set_yticks([])
                ax.text(0.3, 1.04, 'Masked + Bias Frame', transform=ax.transAxes, family='serif', size=6)
                ...
                ax.set_xticks([])
                ax.set_yticks([])
                ax.text(0.28, 1.04, 'Grad of Masked+Bias', transform=ax.transAxes, family='serif', size=6)
