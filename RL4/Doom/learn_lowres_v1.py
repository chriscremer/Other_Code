...
# Heres another prior on the space of reconstrucitons, its about what granularity to model stuff
...
class LOWRES_PREDICTOR(nn.Module):
    def __init__(self):
        super(LOWRES_PREDICTOR, self).__init__()
        ...
        self.deconv3_gp = torch.nn.ConvTranspose2d(in_channels=64, out_channels=3, kernel_size=12, stride=4)#, output_padding=(4,4))
    ...
    def train(self, epochs, DQN, memory, game):
        ...
        for epoch in range(epochs):
            ...
            for learning_step in range(learning_steps):
                ...
                loss, dif = self.forward(batch, DQN)
                loss.backward()
                self.optimizer.step()
                ...
                if total_steps%display_step==0: # and batch_idx == 0:
                    print ('Train Epoch: {}/{}'.format(epoch+1, epochs),
                        ...
                        # 'mask_sum:{:.4f}'.format(mask_sum.data.item()),
                        )
