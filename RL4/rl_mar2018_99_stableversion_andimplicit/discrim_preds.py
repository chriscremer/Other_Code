...
def discrim_predictions(model_dict, rollouts, discriminator, reverse=False):
    ...
    if reverse:
        ...
        states = rollouts.states.index_select(0, idx)
        idx = [i for i in range(n_steps-1, -1, -1)]
        idx = torch.LongTensor(idx).cuda()
        actions = rollouts.actions.index_select(0, idx)
    else:
