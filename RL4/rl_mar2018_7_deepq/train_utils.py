...
def do_gifs(envs, agent, model_dict, update_current_state, update_rewards, total_num_steps):
    ...
    for j in range(n_gifs):
        ...
        while not done_ and step < 400:
            ...
            state_frames.append(np.squeeze(state[0]))
        ...
        for step in range(len(state_frames)-1):
            ...
            if 'Montezuma' in model_dict['env']:
                if action_ in [0]:
                    ...
                    ax.text(.45, -.1, 'Jump', transform=ax.transAxes, color='Red')
                elif action_ in [2]:
                    ax.text(.45, -.1, 'Up', transform=ax.transAxes, color='Blue')
                elif action_ in [3]:
                    ax.text(.45, -.1, 'Right', transform=ax.transAxes, color='Purple')
                elif action_ in [4]:
                    ax.text(.45, -.1, 'Left', transform=ax.transAxes, color='Orange')
                elif action_ in [5]:
                    ax.text(.45, -.1, 'Down', transform=ax.transAxes, color='Green')

                elif action_ in [6]:
                    ax.text(.45, -.1, 'UpRight', transform=ax.transAxes, color='Black')
                elif action_ in [7]:
                    ax.text(.45, -.1, 'UpLeft', transform=ax.transAxes, color='Black')
                elif action_ in [8]:
                    ax.text(.45, -.1, 'DownRight', transform=ax.transAxes, color='Black')
                elif action_ in [9]:
                    ax.text(.45, -.1, 'DownLeft', transform=ax.transAxes, color='Black')

                elif action_ in [10]:
                    ax.text(.45, -.1, 'Up Jump', transform=ax.transAxes, color='Blue')   
                elif action_ in [11]:
                    ax.text(.45, -.1, 'Right Jump', transform=ax.transAxes, color='Black') 
                elif action_ in [12]:
                    ax.text(.45, -.1, 'Left Jump', transform=ax.transAxes, color='Black')
                elif action_ in [13]:
                    ax.text(.45, -.1, 'Down Jump', transform=ax.transAxes, color='Green')


                elif action_ in [14]:
                    ax.text(.45, -.1, 'Up Right Jump', transform=ax.transAxes, color='Black')   
                elif action_ in [15]:
                    ax.text(.45, -.1, 'Up Left Jump', transform=ax.transAxes, color='Black') 
                elif action_ in [16]:
                    ax.text(.45, -.1, 'Down Right Jump', transform=ax.transAxes, color='Black')
                elif action_ in [17]:
                    ax.text(.45, -.1, 'Down Left Jump', transform=ax.transAxes, color='Black')
