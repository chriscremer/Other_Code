...
def update_error_plot(model_dict):
    ...
    # my smoothing
    Vs_smooth = []
    Vs_smooth.append(Vs[0])
    for i in range(1,len(Vs)-1):
        Vs_smooth.append(np.mean(Vs[i-1:i+2]))
    Vs_smooth.append(Vs[-1])


    # my smoothing
    Qs_smooth = []
    Qs_smooth.append(Qs[0])
    for i in range(1,len(Qs)-1):
        Qs_smooth.append(np.mean(Qs[i-1:i+2]))
    Qs_smooth.append(Qs[-1])



    # plt.plot(timesteps, var_reward_sums)
    plt.plot(timesteps, Vs_smooth, label='V')
    plt.plot(timesteps, Qs_smooth, label='Q')
    ...
    fig_path = ls_path + 'errors_V_and_Q'
