...
cross_entropy = -tf.reduce_sum(tf.neg(tf.square(y_*tf.log(tf.clip_by_value(y,1e-10,1.0)))))
# cross_entropy = -tf.reduce_sum((y_*tf.log(tf.clip_by_value(y,1e-10,1.0))))


# debug = tf.neg(tf.square(y_*tf.log(tf.clip_by_value(y,1e-10,1.0))))
# debug = tf.clip_by_value(y,1e-10,1.0)
		...
		with tf.Session() as sess:
			...
			for iter_ in range(999999):
				...
				if iter_ % (10000/batch_size) == 0:
					valid_ce = sess.run(cross_entropy, feed_dict={x: valid_x_alt, y_: valid_y_alt})
				...
				# aaa = sess.run(debug, feed_dict={x: batch[0], y_: batch[1]})
				# print aaa[0]
				# fadsf
...
#save results
with open('results_squared_10.pkl', "wb" ) as f:
