...
grads_and_vars = tf.train.MomentumOptimizer(lr, mom).compute_gradients(cross_entropy)
...
for rep in range(reps):
	...
	result_strore.append([])

	#for different amounts of bias
	for bias in bias_set:

		print 'Bias ' + str(bias) + ' Rep ' + str(rep)
		...
		train_x_alt, train_y_alt = tools.bias_sampling2(train_x, train_y, bias)
		print 'Training set ' +  str(train_x_alt.shape)
		print tools.class_counter(train_y_alt)

		valid_x_alt, valid_y_alt = tools.bias_sampling2(valid_x, valid_y, bias)
		print 'Validation set ' +  str(valid_x_alt.shape)
		print tools.class_counter(valid_y_alt)


		for samp in range(999999):

			ind = samp % len(train_y_alt)

			#check for convergence
			if samp % 10000 == 0:
				valid_ce = sess.run([cross_entropy], feed_dict={x: valid_x_alt, y_: valid_y_alt})[0]
				print samp, valid_ce
				...
				if best_valid_ce - valid_ce < tol and best_valid_ce != -1:
					...
					if early_stop_count == 2:
						...
						# print class_counter(sess.run([y], feed_dict={x: train_x_alt, y_: train_y_alt})[0])
...
#save results
with open('results_regular.pkl', "wb" ) as f:
	pickle.dump(result_store, f)
	print 'saved results ' + str(i)
