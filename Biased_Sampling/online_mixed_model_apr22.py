...
aaa = np.array(class_counter(train_y))
...
def background_model(_X, background_samps, background_target):
	...
	for i in range(len(background_samps)):
		...
		dist = sum(((_X - background_samps[i])**2)[0])
		if len(dists) < 5:
			dists.append(dist)
			indexes.append(i)
		elif max(dists) > dist:
	...
	for i in range(len(dists)):
		pred = pred + ((dists[i]/dist_sum)*background_target[indexes[i]])
...
for i in range(999999):
	...
	_ = sess.run([apply_grad], feed_dict={x: batch[0], y_: batch[1], B: [B_pred]})
