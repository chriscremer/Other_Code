...
def train(exp_dict):
    ...
    for step in range(0, S['max_steps'] + 1):
        ...
        batch = get_batch(data=S['train_x'], batch_size=S['batch_size'])
        warmup = min((step+S['load_step']) / float(S['warmup_steps']), 1.)

        outputs = model.forward(batch, warmup=warmup)
...
if __name__ == "__main__":
    ...
    # def get_batch(image_dataset, question_dataset, batch_size):
