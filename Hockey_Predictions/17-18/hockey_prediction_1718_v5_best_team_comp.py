...
#figure out the best team composition

#so randomly select comp, select players, get avg score for comps


# atm it seems like taking 2 C or LW , 3 of other FWD, 6D , 2G is best

#going to look at which comp is 1st most often, instead of avg score

# i maed a shitty comp, and it does well on avg but bad at coming first.
# it should do poorly on both
	#there is something wrong with computing avg score, so just go with first place count


# it seems like 3G does best.
# either 5 or 6 D

# ok so new strategy, get 2 good goalies early
# get third late

# current order: G,G,D,D,D,F ...
...
def update_team_comp_score(all_comps, comp_scores, comp_count, team_comps, team_scores):

	# print team_comps
	for comp in team_comps:
		comp_index = all_comps.index(comp)
		# print comp_index
		# player_index = all_names.index(name)
		comp_count[comp_index] += 1
		# mean = prev_mean + (prev_mean + new_score) / count
		comp_scores[comp_index] = comp_scores[comp_index] + (team_scores[comp_index]-comp_scores[comp_index])/ comp_count[comp_index]
	


def sim_draft2(team_pos_comps):
	...
	positions_dynamic = ['C', 'LW', 'RW', 'D', 'G']
	# positions_open = [3,3,3,5,2]  #C, LW, RW, D, G
	# positions_open_per_team = list(team_comps)
	teams_statuses = list(team_pos_comps)
...
comp_scores = np.zeros(len(team_comps_list))
comp_count = np.zeros(len(team_comps_list))

comp_first_place_count = np.zeros(len(team_comps_list))
...
for iter_ in range(iter_total):
	...
	#make position comps
	comps = [team_comps_list[np.random.randint(len(team_comps_list))] for x in range(n_teams)] 
	compscopy = copy.deepcopy(comps)

	#Make teams
	team_comps = sim_draft2(compscopy)
	...
	best_index = list(team_overall_score).index(max(team_overall_score))
	best_comp = comps[best_index]
	comp_index = team_comps_list.index(best_comp)
	comp_first_place_count[comp_index] +=1


	#UPdate comp score
	update_team_comp_score(team_comps_list, comp_scores, comp_count, comps, team_overall_score)
...
# print comp_first_place_count

for c in range(len(comp_scores)):
# 	print comp_scores[c], team_comps_list[c]
	print comp_first_place_count[c], team_comps_list[c]
