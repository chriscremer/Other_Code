...
from distributions import lognormal, Flow1
...
def compute_log_like_prior(model, image_dir, attr_dict, n_batches, indexes, inf_type=1, dec_type=0, k=1, prior=None): #, k=1): #, answer_dataset, image_idxs, first_question_idx):
    ...
    with torch.no_grad():
        for i in range(n_batches):
            ...
            img_batch, question_batch = make_batch(image_dir, attr_dict, batch_size=20, indexes=indexes, word_to_idx=word_to_idx)
...
def make_batch_in_order(image_dir, attr_dict, batch_size, indexes, word_to_idx, cur_index): # val=False, SSL=False): #, answer_dataset): #, image_idxs, first_question_idx):
    ...
    return img_batch, text_batch, idx #, answer_batch
...
def eval_qy(model, val_indexes, image_dir, attr_dict, word_to_idx, classifier):
    ...
    # models = ['qy_true_new', 'qy_clevr_true_seed1', 'qy_clevr_true_seed2']
    # models = ['qy_celeba_agg', 'qy_celeba_agg_seed1', 'qy_celeba_agg_seed2']
    models = ['qy_celeba_true', 'qy_celeba_true_seed1', 'qy_celeba_true_seed2']
    ...
    for m in models:
        ...
        qy.load_params_v3(save_dir=path, step=460000)
        ...
        with torch.no_grad():
            ...
            while i+batch_size < n:
                ...
                qy_gen_image_real_words_acc_val = classifier.accuracy_sequence(x=alpha.detach(), attributes=question_batch)
                ...
                correctness_model.append(qy_gen_image_real_words_acc_val)
...
def train2(self, max_steps, load_step, 
    ...
    # indexes_ = train_indexes_2

    indexes_ = train_indexes


    # # if args.model_load_step>0:
    # # dir_to_load = home + "/Documents/VLVAE_exps/celeba_qys_learn_agg/params/" #model_params450000.pt"
    # dir_to_load = home + "/Documents/VLVAE_exps/celeba_qys_learnprior_flow6_2/params/" #model_params450000.pt"
    # model.load_params_v3(save_dir=dir_to_load, step=450000)
    ...
    #         learned_prior_acc = classifier.accuracy_sequence(x=x_samp_learned_prior.detach(), attributes=sampled_words_learned_prior.detach())
    ...
    #     # dfaafda
        ...
        #     val_img_batch, val_question_batch, idx = make_batch_in_order(image_dir, attr_dict, batch_size, indexes=val_indexes, word_to_idx=word_to_idx, cur_index=idx)

        #     logws = []
        #     for j in range(k):
        #         # outputs = model.forward_prior(x=img_batch, q=question_batch, inf_type=inf_type, dec_type=dec_type, prior=prior)
        #         outputs = self.forward_prior(x=val_img_batch, q=val_question_batch, inf_type=2, dec_type=2, prior=prior1) #, k=k) #, answer_batch)
        #         logws.append(outputs['logws'])
        ...
        #     for j in range(k):
        #         # outputs = model.forward_prior(x=img_batch, q=question_batch, inf_type=inf_type, dec_type=dec_type, prior=prior)
        #         outputs = self.forward_prior(x=val_img_batch, q=val_question_batch, inf_type=2, dec_type=2, prior=prior2) #, k=k) #, answer_batch)
        #         logws.append(outputs['logws'])
        ...
        #     if idx % 100==0:
        #         print(idx,len(val_indexes),  'logpy',  np.mean(LLs), np.std(LLs),  LL_x, np.mean(LLs_realprior), np.std(LLs_realprior),  LL_x_real)


        # # print ('avg: ', np.mean(accs))
        # print ('avgLLy: ', np.mean(LLs))
        # print ('avgLLy_real: ', np.mean(LLs_realprior))
    ...
    #     # # LL_x_pz = compute_log_like_prior(model, image_dir, attr_dict, n_batches, indexes, inf_type=1, dec_type=0, k=1, prior=None):
    #     # LL_x_pz = compute_log_like_prior(self, image_dir, attr_dict,  n_batches=20, indexes=val_indexes, inf_type=1, dec_type=1, k=k, prior=prior2)
    #     # LL_y_pz = compute_log_like_prior(self, image_dir, attr_dict,  n_batches=20, indexes=val_indexes, inf_type=2, dec_type=2, k=k, prior=prior2)

    #     # LL_x_qz = compute_log_like_prior(self, image_dir, attr_dict, n_batches=20, indexes=val_indexes, inf_type=1, dec_type=1, k=k, prior=prior1)
    #     # LL_y_qz = compute_log_like_prior(self, image_dir, attr_dict,  n_batches=20, indexes=val_indexes, inf_type=2, dec_type=2, k=k, prior=prior1)
    ...
    #         # if idx % 100==0:
    #             # print (idx,len(val_indexes))

    #         # val_img_batch, val_question_batch = make_batch(val_image_dataset, val_question_dataset, batch_size, val=True)
    #         # make_batch(image_dir, attr_dict, batch_size, indexes=val_indexes, word_to_idx=word_to_idx)
    #         # val_img_batch, val_question_batch, idx = make_batch_in_order(image_dir, attr_dict, batch_size, indexes=val_indexes, word_to_idx=word_to_idx, cur_index=idx)
    #         val_img_batch, val_question_batch, idx = make_batch_in_order(image_dir, attr_dict, batch_size, indexes=indexes_, word_to_idx=word_to_idx, cur_index=idx)
    ...
    #         # acc = classifier.accuracy_sequence(x=training_recon_img.detach(), attributes=val_question_batch)
    #         # accs.append(acc)
    ...
    #         for j in range(k):
    #             # outputs = model.forward_prior(x=img_batch, q=question_batch, inf_type=inf_type, dec_type=dec_type, prior=prior)
    #             outputs = self.forward_prior(x=val_img_batch, q=val_question_batch, inf_type=1, dec_type=1, prior=prior2) #, k=k) #, answer_batch)
    #             logws.append(outputs['logws'])
    ...
    #         # LL_x = compute_log_like_prior2(self, val_image_dataset, val_question_dataset, n_batches=20, indexes=val_indexes, inf_type=1, 
    #         #                                                                                     dec_type=1, k=10, prior=None)
    #         # LLs.append(LL_x)

    #         if idx % 100==0:
    #             print(idx,len(indexes_),  'logpx',  np.mean(LLs), np.std(LLs),  LL_x, np.mean(LLs_realprior), np.std(LLs_realprior),  LL_x_real)
    ...
    for step in range(max_steps):
        ...
        self.eval()
        qy.train()
        qy.qy_optimizer.zero_grad()

        if self.qy_type =='true':
            outputs2 = self.forward_qy2(x=img_batch, q=question_batch, warmup=self.max_beta, inf_type=2, dec_type=2, qy=qy) #, k=k_train) #, marginf_type=marginf_type)
            loss = -outputs2['welbo']

        elif self.qy_type =='agg':
            outputs2 = self.forward_qy2(x=img_batch, q=question_batch, warmup=self.max_beta, inf_type=1, dec_type=0, qy=qy)
            loss = -outputs2['logqzy']
            

        loss.backward()
        qy.qy_optimizer.step()
        ...
        #         # LL_x_pz = compute_log_like_prior(model, image_dir, attr_dict, n_batches, indexes, inf_type=1, dec_type=0, k=1, prior=None):
        #         LL_x_pz = compute_log_like_prior(self, image_dir, attr_dict,  n_batches=20, indexes=val_indexes, inf_type=1, dec_type=1, k=10, prior=prior2)
        #         LL_y_pz = compute_log_like_prior(self, image_dir, attr_dict,  n_batches=20, indexes=val_indexes, inf_type=2, dec_type=2, k=10, prior=prior2)

        #         LL_x_qz = compute_log_like_prior(self, image_dir, attr_dict, n_batches=20, indexes=val_indexes, inf_type=1, dec_type=1, k=10, prior=prior1)
        #         LL_y_qz = compute_log_like_prior(self, image_dir, attr_dict,  n_batches=20, indexes=val_indexes, inf_type=2, dec_type=2, k=10, prior=prior1)
        ...
        #     # z, logpz, logqz = flow.sample(zeros, zeros) 
        ...
        if step_count % display_step == 0:
            ...
            with torch.no_grad():
                ...
                outputs2, alpha, word_preds, sampled_words = self.forward_qy2(x=img_batch, q=question_batch, qy=qy, generate=True)
                qy_real_image_gen_words_acc = classifier.accuracy_sequence(x=img_batch, attributes=sampled_words.detach())
                qy_gen_image_real_words_acc = classifier.accuracy_sequence(x=alpha.detach(), attributes=question_batch)
                qy_gen_image_gen_words_acc = classifier.accuracy_sequence(x=alpha.detach(), attributes=sampled_words.detach())

                logpx_give_y_train = outputs2['logpx_give_y']
                ...
                qy_real_image_gen_words_acc_val = classifier.accuracy_sequence(x=val_img_batch, attributes=sampled_words.detach())
                qy_gen_image_real_words_acc_val = classifier.accuracy_sequence(x=alpha.detach(), attributes=val_question_batch)
                qy_gen_image_gen_words_acc_val = classifier.accuracy_sequence(x=alpha.detach(), attributes=sampled_words.detach())
                ...
                # # print (val_recon_sampled_words.detach().shape)
            ...
            add_to_dict(recent_dict, 'qy_real_image_gen_words_acc', qy_real_image_gen_words_acc)
            add_to_dict(recent_dict, 'qy_gen_image_real_words_acc', qy_gen_image_real_words_acc)
            add_to_dict(recent_dict, 'qy_gen_image_gen_words_acc', qy_gen_image_gen_words_acc)

            add_to_dict(recent_dict, 'qy_real_image_gen_words_acc_val', qy_real_image_gen_words_acc_val)
            add_to_dict(recent_dict, 'qy_gen_image_real_words_acc_val', qy_gen_image_real_words_acc_val)
            add_to_dict(recent_dict, 'qy_gen_image_gen_words_acc_val', qy_gen_image_gen_words_acc_val)
            ...
            print (
                ...
                # 'learned_prioracc:{:.4f}'.format(learned_prior_acc),
            ...
            if step_count > limit_numb:
                ...
                all_dict['qy_real_image_gen_words_acc'].append(np.mean(recent_dict['qy_real_image_gen_words_acc']))
                all_dict['qy_gen_image_real_words_acc'].append(np.mean(recent_dict['qy_gen_image_real_words_acc']))
                all_dict['qy_gen_image_gen_words_acc'].append(np.mean(recent_dict['qy_gen_image_gen_words_acc']))

                all_dict['qy_real_image_gen_words_acc_val'].append(np.mean(recent_dict['qy_real_image_gen_words_acc_val']))
                all_dict['qy_gen_image_real_words_acc_val'].append(np.mean(recent_dict['qy_gen_image_real_words_acc_val']))
                all_dict['qy_gen_image_gen_words_acc_val'].append(np.mean(recent_dict['qy_gen_image_gen_words_acc_val']))
        ...
        if step_count%viz_steps==0:# and total_steps>6:
            ...
            with torch.no_grad():
                ...
                xs = [alpha, alpha, alpha]
                xs_1 = [alpha, alpha, alpha]
                ys = [sampled_words,sampled_words,sampled_words]
                ys_2 = [sampled_words,sampled_words,sampled_words]
        ...
        if step_count % trainingplot_steps==0 and step_count > 0 and len(all_dict['all_steps']) > 2:

            plot_curves_qy(model, save_dir, all_dict)
...
def plot_curves_qy(self, save_dir, all_dict):
    ...
    def make_curve_subplot(self, rows, cols, row, col, steps, values_list, label_list, ylabel, show_ticks, colspan, rowspan, set_xlabel=False, color_list=None, linestyle_list=None, title=0):
        ...
        if title:
            ax.set_title('Exp:'+self.exp_name + '  Dataset:celebA'  + '    Joint_Inf:' + str(self.joint_inf) +
    ...
    make_curve_subplot(self, rows, cols, row=row, col=col, steps=steps, 
        values_list=[all_dict['loss'], all_dict['loss']], 
        label_list=['Train: {:.2f}'.format( all_dict['loss'][-1]), 
                    'Valid: {:.2f}'.format( all_dict['loss'][-1])], 
        ylabel='Loss', show_ticks=True, colspan=text_col_width, rowspan=1, title=1)
    row+=1
    ...
    make_curve_subplot(self, rows, cols, row=row, col=col, steps=steps, 
        values_list=[all_dict['qy_real_image_gen_words_acc'], all_dict['qy_gen_image_real_words_acc'], 
                    all_dict['qy_gen_image_gen_words_acc'],
                    all_dict['qy_real_image_gen_words_acc_val'], all_dict['qy_gen_image_real_words_acc_val'], 
                    all_dict['qy_gen_image_gen_words_acc_val'],
                    ], 
        label_list=[r'$y gen$:   '+'{:.4f}'.format(all_dict['qy_real_image_gen_words_acc'][-1]), 
                    r'$x gen$:   '+'{:.4f}'.format( all_dict['qy_gen_image_real_words_acc'][-1]),
                    r'$both gen$:   '+'{:.4f}'.format(all_dict['qy_gen_image_gen_words_acc'][-1]),
                    r'$y gen val$:   '+'{:.4f}'.format(all_dict['qy_real_image_gen_words_acc_val'][-1]), 
                    r'$x gen val$:   '+'{:.4f}'.format( all_dict['qy_gen_image_real_words_acc_val'][-1]),
                    r'$both gen val$:   '+'{:.4f}'.format(all_dict['qy_gen_image_gen_words_acc_val'][-1]),
                    ], 
        color_list=[color_defaults[0], color_defaults[1],
                    color_defaults[2], color_defaults[3], 
                    color_defaults[4], color_defaults[5],
                    ],
        linestyle_list=['-','-',
                        '-','-','-',
                        '-',
...
if __name__ == "__main__":
    ...
    if args.eval_qy:
        ...
        eval_qy(model, val_indexes, image_dir, attr_dict, word_to_idx, classifier)
