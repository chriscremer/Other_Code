...
def compute_log_like_prior2(model, image_dir, attr_dict, n_batches, indexes, inf_type=1, dec_type=0, k=1, prior=None): #, k=1): #, answer_dataset, image_idxs, first_question_idx):
...
def train2(self, max_steps, load_step, 
    ...
    if self.qy_type =='true' or self.qy_type =='agg':
        ...
        # q(z|y)
        self.encoder_rnn2_2 = nn.GRU(input_size=self.embed_size, hidden_size=self.y_enc_size, num_layers=1, 
                                    dropout=0, batch_first=True, bidirectional=False).cuda()
        self.qzy_fc1_2 = nn.Linear(self.y_enc_size, self.linear_hidden_size).cuda()
        self.qzy_bn1_2 = nn.BatchNorm1d(self.linear_hidden_size).cuda()
        self.qzy_fc2_2 = nn.Linear(self.linear_hidden_size, self.linear_hidden_size).cuda()
        self.qzy_fc3_2 = nn.Linear(self.linear_hidden_size, self.z_size*2).cuda()
        ...
        y_inf_net_params = [list(self.encoder_rnn2_2.parameters()) + list(self.qzy_fc1_2.parameters()) 
                            + list(self.qzy_bn1_2.parameters()) + list(self.qzy_fc2_2.parameters())
    ...
    if eval__:
        ...
        if self.qy_type =='agg':
            # dir_to_load = home + "/Documents/VLVAE_exps/celeba_qys_learn_agg/params/" #model_params450000.pt"
            dir_to_load = home + "/Documents/VLVAE_exps/celeb_train_qy_agg_400k/params/" #model_params450000.pt"            
        if self.qy_type =='true':
            dir_to_load = home + "/Documents/VLVAE_exps/celeba_qys_learn_true/params/"


        # model.load_params_v3(save_dir=dir_to_load, step=350000)
        model.load_params_v3(save_dir=dir_to_load, step=300000)
        # model.load_params_v3(save_dir=dir_to_load, step=650000)
        ...
        with torch.no_grad():
            ...
            while idx < len(indexes_):
                ...
                if idx % 100==0:
                    print(idx,len(indexes_), 'avg acc:', np.mean(accs), acc,  'logpx',  np.mean(LLs), np.std(LLs))#,  LL_x)
    ...
    for step in range(max_steps):
        ...
        if self.qy_type =='true':
            ...
            outputs = self.forward_qy2(x=None, q=question_batch, warmup=self.max_beta, inf_type=2, dec_type=2) #, k=k_train) #, marginf_type=marginf_type)
            loss = -outputs['welbo']
            loss.backward()
            qy_optimizer.step()
            ...
            qy_optimizer.zero_grad()
            # with torch.no_grad():
            outputs = self.forward_qy2(x=img_batch, q=question_batch, warmup=self.max_beta, inf_type=1, dec_type=0)
            loss = -outputs['logqzy']
            loss.backward()
            qy_optimizer.step()
        ...
        if step_count % display_step == 0:
            ...
            with torch.no_grad():
                ...
                LL_x = compute_log_like_prior2(self, image_dir, attr_dict, n_batches=20, indexes=val_indexes, inf_type=1, dec_type=1, k=10, prior=None)

                LL_x_train = compute_log_like_prior2(self, image_dir, attr_dict, n_batches=20, indexes=train_indexes, inf_type=1, dec_type=1, k=10, prior=None)
                ...
                y_recon_acc_val = classifier.accuracy_sequence(x=training_recon_img.detach(), attributes=val_question_batch)
            ...
            if self.qy_type == 'agg':
                print (
                    ...
                    'logpx|y_train:{:.4f}'.format(LL_x_train),
        ...
        #         prior1 = lambda xx: flow.logprob(xx, zeros, zeros)
        #         prior2 = lambda xx: lognormal(xx, zeros, zeros)
