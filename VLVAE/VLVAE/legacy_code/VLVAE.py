...
class VLVAE(nn.Module):
    def __init__(self, kwargs):
        ...
        if self.joint_inf:
            ...
            # decode0_params = [x_inf_params[0] + decoder_params[0] ] # + y_inf_net_params[0] ]  
            self.optimizer_y = optim.Adam(y_inf_net_params[0] , lr=lr, weight_decay=.0000001)
            self.scheduler_y = lr_scheduler.StepLR(self.optimizer_y, step_size=1, gamma=0.999995)
    ...
    def forward(self, x=None, q=None, warmup=1., generate=False, inf_type=1, dec_type=0): #, k=1): #, marginf_type=0):
        ...
        if dec_type == 0: 
            ...
            warmed_elbo = torch.mean(logpx + logpy + warmup*(logqzy - logqz))
