...
def compute_log_like_prior2(model, image_dataset, question_dataset, n_batches, indexes, inf_type=1, dec_type=0, k=1, prior=None): #, k=1): #, answer_dataset, image_idxs, first_question_idx):
    ...
    with torch.no_grad():
        for i in range(n_batches):
            ...
            logws = []
            for j in range(k):
                outputs = model.forward_prior(x=img_batch, q=question_batch, inf_type=inf_type, dec_type=dec_type, prior=prior1) #, k=k) #, answer_batch)
                logws.append(outputs['logws'])
...
def train2(self, max_steps, load_step, 
    ...
    if self.qy_type =='true' or self.qy_type =='agg':
        ...
        # if self.qy_type =='agg':
        #     dir_to_load = home + "/Documents/VLVAE_exps/train_learn_qy_agg_betterqy/params/" #model_params450000.pt"
        # if self.qy_type =='true':
        #     dir_to_load = home + "/Documents/VLVAE_exps/train_learn_qy_true_betterqy/params/"
        ...
        #         accs.append(acc.cpu().data.numpy())
    ...
    # # if args.model_load_step>0:
    # dir_to_load = home + "/Documents/VLVAE_exps/train_learn_qy_agg_betterqy/params/"
    # model.load_params_v3(save_dir=dir_to_load, step=800000)
    ...
    #         outputs, training_recon_img, training_recon_q, training_recon_sampled_words = self.forward_qy2(x=val_img_batch, q=val_question_batch, warmup=self.max_beta, inf_type=2, dec_type=0, generate=True)
    #         classi_loss_prior, prior_acc = classifier.classifier_loss(x=training_recon_img.detach(), attributes=val_question_batch)

            
    #         accs.append(prior_acc.data.item())


    #         LL_x = compute_log_like_prior2(self, val_image_dataset, val_question_dataset, n_batches=20, indexes=val_indexes, inf_type=1, dec_type=1, k=10, prior=None)
    #         LLs.append(LL_x)

    #         print(i,prior_acc,LL_x)
    ...
    for step in range(max_steps):
        ...
        if not self.just_classifier:
            ...
            if self.joint_inf:
                ...
                #     # self.optimizer_x.zero_grad()
                #     with torch.no_grad():
                #         outputs = self.forward(x=img_batch, q=question_batch, warmup=self.max_beta, inf_type=1, dec_type=0) #, k=k_train) #, marginf_type=marginf_type)
                #         # loss = -outputs['welbo']
                #         # loss.backward()
                #         # self.optimizer_x.step()
                #         # self.scheduler_x.step()
                ...
                #     logqzy = flow.logprob(zs, zeros, zeros)
                #     # print (logqzy.shape)
                ...
                # if step_count % display_step == 0:
                    ...
                    #     #     # of learned prior
                    #     #     entropy_learned_prior = get_entropy(flow, k_entropy=100)
                    #     #     # of real prior 
                    ...
                    #     #     logpz_learned = torch.mean(flow.logprob(z_val, zeros, zeros))
                    #     #     logpz = torch.mean(lognormal(z_val, zeros, zeros)) #[B]

                    #     # embed = self.encoder_embed(q)
                    #     # y_enc = self.encode_attributes2_2(embed)
                    #     # mu, logvar = self.inference_net_y_2(y_enc)
                    #     # prior1 = lambda xx: flow_cond_2.logprob(xx, mu, logvar, y_enc)

                    #     zeros = torch.zeros(batch_size, self.z_size).cuda()
                    #     prior2 = lambda xx: lognormal(xx, zeros, zeros)

                    #     LL_x = compute_log_like_prior2(self, val_image_dataset, val_question_dataset, n_batches=20, indexes=val_indexes, inf_type=1, dec_type=1, k=10, prior=None)
                    #     LL_y = compute_log_like_prior(self, val_image_dataset, val_question_dataset, n_batches=20, indexes=val_indexes, inf_type=2, dec_type=2, k=10, prior=prior2)
                    ...
                    # if self.qy_type == 'agg':
                    #     print (
                    #         'ent:{:.4f}'.format(ent.data.item()),
                    #         'logpx|y:{:.4f}'.format(LL_x),
                    #         'logpy:{:.4f}'.format(LL_y),
                    #         # 'logpx_learned:{:.4f}'.format(LL_x_qz),
                    #         # 'logpy_learned:{:.4f}'.format(LL_y_qz),
                    #         # 'logqzy:{:.4f}'.format(outputs2['logqzy'].data.item()),
                    ...
                    #         # 'logpx_learned:{:.4f}'.format(LL_x_qz),
                    #         # 'logpy_learned:{:.4f}'.format(LL_y_qz),
                    #         # 'logqzy:{:.4f}'.format(outputs['logqzy'].data.item()),
                    #         # 'lqz:{:.4f}'.format(outputs2['logqz'].data.item()),
                    #         )
        ...
        if step_count % display_step == 0:
            ...
            with torch.no_grad():
                ...
                _, qy_real_image_gen_words_acc = classifier.classifier_loss(x=img_batch, attributes=sampled_words.detach())
                _, qy_gen_image_real_words_acc = classifier.classifier_loss(x=alpha.detach(), attributes=question_batch)
                _, qy_gen_image_gen_words_acc = classifier.classifier_loss(x=alpha.detach(), attributes=sampled_words.detach())
                ...
                # # _, y_recon_acc = classifier.classifier_loss(x=img_batch, attributes=sampled_words.detach())
                # # _, y_recon_acc = classifier.classifier_loss(x=img_batch, attributes=sampled_words.detach())
                ...
                # xs = []
                # ys_2 = []
                # for i in range(n_samps_for_plotting):
            ...
            add_to_dict(recent_dict, 'qy_gen_image_gen_words_acc_val', qy_gen_image_gen_words_acc_val.data.item())
            ...
            # add_to_dict(recent_dict, 'acc0', acc0.data.item())
            # add_to_dict(recent_dict, 'acc1', acc1.data.item())
            # add_to_dict(recent_dict, 'acc2', acc2.data.item())
            # add_to_dict(recent_dict, 'acc3', acc3.data.item())

            # add_to_dict(recent_dict, 'acc0_prior', acc0_prior.data.item())
            # add_to_dict(recent_dict, 'acc1_prior', acc1_prior.data.item())
            # add_to_dict(recent_dict, 'acc2_prior', acc2_prior.data.item())
            # add_to_dict(recent_dict, 'acc3_prior', acc3_prior.data.item())
            ...
            print (
                ...
                # # 'zavgvar:{:.2f}'.format(np.mean(z_avg_var)),
                # 'acc:{:.2f}'.format(np.mean(y_recon_acc.data.item())),
                # 'yinf_acc_e:{:.4f}'.format(np.mean(recent_y_inf_acc_entangle)),
                )
        ...
        if step_count % trainingplot_steps==0 and step_count > 0 and len(all_dict['all_steps']) > 2:
            ...
            # print (len(all_dict['all_train_elbos']))
