...
if __name__ == "__main__":
    ...
    #get an image. 
    img1 = torch.from_numpy(train_image_dataset[i1]).cuda().view(1,3,112,112)
    # print (torch.max(img1))
    img1 = torch.clamp(img1, min=1e-5, max=1-1e-5)

    img2 = torch.from_numpy(train_image_dataset[i2]).cuda().view(1,3,112,112)
    img2 = torch.clamp(img2, min=1e-5, max=1-1e-5)

    # print (img1.shape)

    print (get_sentence(train_question_dataset[i1]))
    print (get_sentence(train_question_dataset[i2]))
    # fsd

    # t1 = get_sentence(train_question_dataset[i1])
    # for i in range(1, len(train_question_dataset)):
    #     if get_sentence(train_question_dataset[i]) == t1:
    #         print (i)
    # fsf
    ...
    with torch.no_grad():
        ...
        #     #encode it 
        #     mu, logvar = model.inference_net_x(model.image_encoder2(img1))
        #     print (mu.shape)
        #     fsd

        mu, logvar = model.inference_net_x(model.image_encoder2(img1))
        z1=mu
        z_dec = model.z_to_enc(z1)
        x_hat = model.image_decoder(z_dec)
        recon = torch.sigmoid(x_hat)




        mu, logvar = model.inference_net_x(model.image_encoder2(img2))
        z2=mu
        z_dec = model.z_to_enc(z2)
        x_hat = model.image_decoder(z_dec)
        recon2 = torch.sigmoid(x_hat)


        dif = z2-z1
        
        change = dif / n_interps
        interps = []

        # print (z1, 'z1')
        for i in range(n_interps):
            interp = z1+(i+1)*change

            z_dec = model.z_to_enc(interp)
            x_hat = model.image_decoder(z_dec)
            rec = torch.sigmoid(x_hat)
        ...
        # z3 = (z1+z2) /2.
        # z_dec = model.z_to_enc(z3)
        # x_hat = model.image_decoder(z_dec)
        # inter3 = torch.sigmoid(x_hat)
    ...
    col=0
    row=0
    make_image_subplot(rows, cols, row, col, img1[0], text='')

    col=0
    row=1
    make_image_subplot(rows, cols, row, col, recon[0], text='')

    for i in range(n_interps):

        col=1+i
        row=1
        make_image_subplot(rows, cols, row, col, interps[i][0], text='')


    # col=2
    # row=0
    # make_image_subplot(rows, cols, row, col, img2[0], text='')

    col= n_interps
    row=0
    make_image_subplot(rows, cols, row, col, img2[0], text='')

    plt_path = images_dir + 'interpolate.png'
    plt.savefig(plt_path)
    print ('saved viz',plt_path)
    plt.close(fig)
