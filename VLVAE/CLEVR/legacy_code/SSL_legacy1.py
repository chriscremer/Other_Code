...
def compute_log_like2(model, image_dataset, question_dataset, n_batches, indexes, inf_type=1, dec_type=0, k=1): #, k=1): #, answer_dataset, image_idxs, first_question_idx):
    ...
    with torch.no_grad():
        ...
        for i in range(n_batches):
            ...
            # print (logws.shape)
            # fdfa
            LLs.append( torch.mean(logmeanexp(logws)).data.item()) # outputs['elbo'].data.item())
...
def train2(self, max_steps, load_step, 
    ...
    recent_SSL1 = deque(maxlen=10)
    recent_SSL2 = deque(maxlen=10)
    ...
    for step in range(max_steps):
        ...
        if not self.just_classifier:
            ...
            if self.joint_inf:
                ...
                loss_only_x = -outputs_only_x['welbo'] #*.2
                loss_only_x.backward()
                self.optimizer_qz_x.step()
                self.scheduler_qz_x.step()
                ...
                self.optimizer_qz_x.zero_grad()
                outputs_only_x = self.forward(x=img_batch, q=None, warmup=warmup, inf_type=1, dec_type=1) #, k=k_train)
                loss_only_x = -outputs_only_x['welbo']  *.1
                loss_only_x.backward()
                self.optimizer_qz_x.step()
                self.scheduler_qz_x.step()
        ...
        if step_count % display_step == 0:
            ...
            with torch.no_grad():
                ...
                SSL_LL1 = compute_log_like2(model, train_image_dataset, train_question_dataset, n_batches=1, inf_type=2, dec_type=2, indexes=unpaired_xs_1_indexes, k=1)
                SSL_LL2 = compute_log_like2(model, train_image_dataset, train_question_dataset, n_batches=1, inf_type=2, dec_type=2, indexes=unpaired_xs_2_indexes, k=1)
            ...
            recent_SSL1.append(SSL_LL1)
            recent_SSL2.append(SSL_LL2)
            ...
            print (
                ...
                'SSL_elbo:{:.4f}'.format(SSL_outputs2['elbo'].data.item()),
                'SSL_pyz:{:.4f}'.format(SSL_outputs2['logpy'].data.item()),
                'SSL_pz:{:.4f}'.format(SSL_outputs2['logpz'].data.item()),
                'SSL_qz:{:.4f}'.format(SSL_outputs2['logqz'].data.item()),
                )
            ...
            if step_count > limit_numb:
                ...
                all_dict['SSL1'].append(np.mean(recent_SSL1))
                all_dict['SSL2'].append(np.mean(recent_SSL2))
...
def plot_curves(self, save_dir, all_dict):
    ...
    make_curve_subplot(self, rows, cols, row=row, col=col, steps=steps, 
        values_list=[all_dict['SSL1'], all_dict['SSL2']], 
        label_list=['SSL1: {:.2f}'.format( all_dict['SSL1'][-1]),
                    'SSL2: {:.2f}'.format( all_dict['SSL2'][-1])], 
        ylabel='weighted logp(y) lower bound', show_ticks=False, colspan=text_col_width, rowspan=1)
    row+=1
...
if __name__ == "__main__":
    ...
    if quick_check:
        ...
        trainingplot_steps = 500# 5000 # debug_steps #2000 # 4 #1000# 50 #100# 1000 # 500 #2000
