    ...
    for step in range(max_steps):
        ...
        if not self.just_classifier:
            ...
            if self.joint_inf:
                ...
                torch.nn.utils.clip_grad_norm_(self.decode0_params[0], .5)
                ...
                #     # outputs = self.forward(x=img_batch, q=question_batch, warmup=warmup, inf_type=1, dec_type=0) #, k=k_train) #, marginf_type=marginf_type)
                #     # z_prior = self.sample_prior(B=self.B)
                #     z_prior = torch.FloatTensor(self.B, self.z_size).normal_().cuda()
                #     discrim_loss, discrim_acc = self.discrim.discrim_loss(outputs['z'].detach(), z_prior)
                #     # loss = -outputs['welbo']
                ...
                for jj in range(5):
                    ...
                    # z_prior = self.sample_prior(B=self.B)
                    z_prior = torch.FloatTensor(self.B, self.z_size).normal_().cuda()
                    discrim_loss, disc_fake, disc_real, grad_penalty = self.discrim.discrim_loss(z_prior, outputs['z'].detach())
                    # loss = -outputs['welbo']
                    discrim_loss.backward()
                    self.discrim_opt.step()
                    # self.scheduler_x.step()
                ...
                # if step%100 == 0:
                #     print (discrim_loss.data.item(), grad_penalty.data.item(), 'Fake:', disc_fake.data.item(), ' Real:', disc_real.data.item())
                #     fake
        ...
        if step_count % display_step == 0:
            ...
            add_to_dict(recent_dict, 'discrim_loss', discrim_loss.data.item())
            # add_to_dict(recent_dict, 'discrim_acc', discrim_acc.data.item())
            ...
            print (
                ...
                'discrim_loss:{:.4f}'.format(recent_dict['discrim_loss'][-1]),
                # 'discrim_acc:{:.4f}'.format(recent_dict['discrim_acc'][-1]),
                'z var:{:.4f}'.format(torch.mean(torch.var(outputs['z']), 0).data.item()),
                'z max:{:.4f}'.format(torch.max(outputs['z']).data.item()),
                'z min:{:.4f}'.format(torch.min(outputs['z']).data.item()),
                'grad_pen:{:.2f}'.format(np.mean(grad_penalty.data.item())),
                'fake:{:.2f}'.format(np.mean(disc_fake.data.item())),
                'real:{:.2f}'.format(np.mean(disc_real.data.item())),
                )
...
def plot_curves(self, save_dir, all_dict):
    ...
    make_curve_subplot(self, rows, cols, row=row, col=col, steps=steps, 
        values_list=[all_dict['discrim_loss']], 
        label_list=[': {:.2f}'.format( all_dict['aa_prior_classifier_acc'][-1])], 
        ylabel='Discrim Error', show_ticks=False, colspan=text_col_width, rowspan=1)
    row+=1
...
def vizualize_2D(self, step):
    ...
    ax.scatter(x=zs[:,0],y=zs[:,1], c='blue', s=6, alpha=.3)
    ...
    ax.scatter(x=zs_11[:,0],y=zs_11[:,1], c='blue', s=6, alpha=.3)
    ax.scatter(x=zs[:,0],y=zs[:,1], c='red', s=6, alpha=.3)
