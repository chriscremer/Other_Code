...
def plot_isocontours2_agg(ax, indexes, model, xlimits=[-6, 6], ylimits=[-6, 6],
    ...
    # Z = np.clip(Z, a_min=.00051, a_max=.02)
    Z = np.clip(Z, a_min=0., a_max=.0036)
...
def train2(self, max_steps, load_step, 
    ...
    with torch.no_grad():
        ...
        if v2:
            ...
            levels = np.linspace(0.0001,.0035, 20)
            legend1 = False
            k=2
            # x_lims = [-2.6,2.6]
            # y_lims = [-1,.5]
            ...
            rows = 1
            cols = 1
            fig = plt.figure(figsize=(4+cols,6+rows), facecolor='white', dpi=150)
            ...
            # plot_isocontours2_agg(ax, red_indexes, model=model, xlimits=x_lims, ylimits=y_lims,
            #                  numticks=51, cmap='Greens', alpha=1., legend=legend1, levels=levels)
            ...
            # plot_isocontours2_agg(ax, red_indexes, model=model, xlimits=x_lims, ylimits=y_lims,
            #                  numticks=101, cmap='Oranges', alpha=1., legend=legend1, levels=levels)
            ...
            # plot_isocontours2_agg(ax, red_indexes, model=model, xlimits=x_lims, ylimits=y_lims,
            #                  numticks=101, cmap='Greens', alpha=1., legend=legend1, levels=levels)
            ...
            plot_isocontours2_agg(ax, red_indexes[:10], model=model, xlimits=x_lims, ylimits=y_lims,
                             numticks=101, cmap='Greens', alpha=1., legend=legend1, levels=levels)
            ...
            plot_isocontours2_agg(ax, red_indexes[:10], model=model, xlimits=x_lims, ylimits=y_lims,
                             numticks=101, cmap='Blues', alpha=1., legend=legend1, levels=levels)
            ...
            plot_isocontours2_exp_norm(ax, func, xlimits=x_lims, ylimits=y_lims,
                             numticks=101, cmap='Blues', alpha=.5, legend=False, contourtype='notf')
            # ax.grid(True, alpha=.3)
            ax.tick_params(axis='x', colors='white')
            ax.yaxis.set_tick_params(labelsize=5)
            ax.tick_params(axis='y', colors='white')
            ...
            func = lambda zs: logposterior_func_true(self, question_batch[0], zs)
            plot_isocontours2_exp_norm(ax, func, xlimits=x_lims, ylimits=y_lims,
                             numticks=101, cmap='Greens', alpha=.5, legend=False, contourtype='notf')
            # # ax.grid(True, alpha=.3)
            # ax.tick_params(axis='x', colors='white')
            # ax.yaxis.set_tick_params(labelsize=5)
            # ax.tick_params(axis='y', colors='white')
