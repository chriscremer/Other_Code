...
def run_mongo_query(query, numb):
	...
	for iter1 in range(10):
		...
		if numb == 1:
			...
			query_output == db.collection.group(
						{"thousandth" : True},
						{"$and": [{"num" : { "$gte" : 1234 } },
						          {"num" : { "$lt"  : 5678 } }]},
						{ "total" : 0 },
...
argo3_queries = [
					...
					"DROP TABLE IF EXISTS temp_argo; CREATE TEMP TABLE temp_argo AS SELECT objid FROM argo_a1_num WHERE keystr = 'num' and valnum > 1234 AND valnum < 5678; SELECT count(*) FROM argo_a1_num WHERE objid in (SELECT objid FROM temp_argo) AND keystr = 'thousandth' GROUP BY valnum"
