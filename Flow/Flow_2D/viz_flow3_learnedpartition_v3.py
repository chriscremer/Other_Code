...
class flow1(nn.Module):
    ...
    def __init__(self, n_flows):
        ...
        self.baseline = torch.zeros(1)  #, requires_grad=True) * .5
        self.baseline.requires_grad_()
    ...
    def sample(self, N=1, n_flows=-1, temp=100.):
        ...
        logprobs = 0
        for i in range(n_flows):
            z, logdet, logprob = self.transform(z, pre_mask=self.masks[i], net=self.nets[i], temp=temp)
            logdetsum += logdet
            logprobs += logprob
        zT = z

        logprob = logqz0 - logdetsum
        return zT, logprob.view(N), logprobs
    ...
    def logprob(self, z, n_flows=-1, temp=100.):
        ...
        for i in list(range(n_flows))[::-1]:
            z, logdet, logprob = self.transform(z, pre_mask=self.masks[i], net=self.nets[i], reverse=True, temp=temp)
            logdetsum += logdet
        z0 = z
...
if __name__ == "__main__":
    ...
    for i in range(20000):
        ...
        z, logprob, logprob_mask = flow.sample(batch_size) #, temp=temp)
        logtarget = log_targetdist(z) #.detach()
        # print (logprobs.shape)
        # fasd
        logprob_mask = torch.mean(logprob_mask)

        l1 = torch.mean(logprob - logtarget)
        l2 = (l1.detach() - flow.baseline).detach() * logprob_mask 
        l3 = (flow.baseline - l1.detach())**2
        ...
        if i%100 == 0:
            print (i, numpy(loss), 'l1', numpy(l1), 'l2', numpy(l2), 'l3', numpy(l3), 'baseline', numpy(flow.baseline)) #, 'temp:', temp)

        if i %999 ==0:
            # print (torch.sigmoid(flow.masks[0]),  torch.sigmoid(flow.masks[1]), ) #torch.sigmoid(flow.masks[2]))
            for i in range(n_flows):
                print (numpy(torch.sigmoid(flow.masks[i])))
    ...
    for i in range(n_flows+1):
        ...
        for n in range(n_samps):
            z, logprob, mask_logprob = flow.sample(n_flows=i, temp=-1)
