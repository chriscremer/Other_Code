...
# os.environ['CUDA_VISIBLE_DEVICES'] = '1' #args.which_gpu



exp_dir = args.save_dir + args.exp_name + '/'
...
for epoch in range(args.max_epochs):
    ...
    for batch_idx, (input,_) in enumerate(train_loader):
        ...
        if (batch_idx +1) % args.print_every == 0 : 
            ...
            print('{:3d}, loss : {:.4f}, time : {:.4f}'.format(
                epoch,
                (train_loss / deno), 
                (time.time() - time_)))
            train_loss = 0.
    ...
    with torch.no_grad():
        ...
        if (epoch + 1) % args.save_interval == 0: 
            torch.save(model.state_dict(), params_dir+'{}_{}.pth'.format(model_name, epoch))
            print('sampling...')
            sample_t = sample(model)
            sample_t = rescaling_inv(sample_t)
            utils.save_image(sample_t,images_dir+'{}_{}.png'.format(model_name, epoch), nrow=5, padding=0)
