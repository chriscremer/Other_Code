...
class PixelCNN(nn.Module):
    def __init__(self, nr_resnet=5, nr_filters=80, nr_logistic_mix=10, 
        ...
        # self.ul_init = nn.ModuleList([down_shifted_conv2d(input_channels+1, nr_filters, filter_size=(1,3), shift_output_down=True), 
        #                                down_right_shifted_conv2d(input_channels + 1, nr_filters, 
        #                                     filter_size=(2,1), shift_output_right=True)])
        self.ul_init = nn.ModuleList([down_shifted_conv2d(input_channels, nr_filters, filter_size=(1,3), shift_output_down=True), 
                                       down_right_shifted_conv2d(input_channels, nr_filters, filter_size=(2,1), shift_output_right=True)])

        num_mix = 3 if self.input_channels == 1 else 10

        # self.nin_out = nin(nr_filters, num_mix * nr_logistic_mix)
        self.nin_out = nin(nr_filters, input_channels*2)
