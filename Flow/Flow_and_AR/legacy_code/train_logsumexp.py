...
for i in range(max_steps+1):
    ...
    if step %args.print_every == 0:
        ...
        myprint( (step, 'T:{:.1f}'.format(T), 
                'LME:{:.2f}'.format(float(nobj_lme.data.cpu().numpy())), 
                'BPD:{:.2f}'.format(float(nobj.data.cpu().numpy())), 
                'lr', lr_sched.get_lr()[0]) ) 
        t = time.time()
        ...
        if step > 5:
            ...
            data_dict['logmeanexp'].append(float(nobj_lme.data.cpu().numpy()))
