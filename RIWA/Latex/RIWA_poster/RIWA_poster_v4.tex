...
The variational autoencoder (VAE; \cite{vae}) maximizes the following evidence lower bound (ELBO):
\begin{align}
    log(p(x)) & \geq E_{z \sim q(z|x)} \left[  %\frac{1}{k}\sum_{i=1}^k
    log\left(\frac{p(x,z)}{q(z|x)}  \right)  \right]  \nonumber \\
    &= L_{VAE}[q]. \nonumber %\label{vae_elbo}  %\eqname{(VAE ELBO)}
\end{align}

The importance-weighted autoencoder (IWAE; \cite{burda2015importance}) maximizes the following tighter multi-sample lower bound: 
\begin{align} 
    log(p(x)) & \geq E_{z_{1}...z_{k} \sim q(z|x)} \left[log\left(  \frac{1}{k}  \sum_{i=1}^k \frac{p(x,z_i)}{q(z_i|x)}  \right)  \right]  \nonumber \\
    &= L_{IWAE}[q]. \nonumber %\label{iwae_elbo}  %\eqname{(IWAE ELBO)}
\end{align}
...
% the following multi-sample evidence lower bound (ELBO): 
% \begin{align} 
%     log(p(x)) & \geq E_{z_{1}...z_{k} \sim q(z|x)} \left[log\left(  \frac{1}{k}  \sum_{i=1}^k \frac{p(x,z_i)}{q(z_i|x)}  \right)  \right]  \nonumber \\
%     &= L_{IWAE}[q] \nonumber %\label{iwae_elbo}  %\eqname{(IWAE ELBO)}
% \end{align}
...
% $f_\phi(x)$ is a recognition network parametrized by $\phi$ and 
$Cat(\tilde{w})$ refers to a categorical distribution parametrized by $\tilde{w}$. 
\end{textblock}
...
\begin{align} 
    q_{IW}(z_i|x,z_{\setminus i}) &= k \tilde{w}_i q(z_i|x)
    = \left( \frac{ \frac{p(x,z_i)}{q(z_i|x)}}{  \frac{1}{k}   \sum_{j=1}^k \frac{p(x,z_j)}{q(z_j|x)}}  \right) q(z_i|x) = \frac{p(x,z_i)}{\frac{1}{k}   \sum_{j=1}^k \frac{p(x,z_j)}{q(z_j|x)}} \nonumber
\end{align}
...
Note that, to evaluate $q_{IW}$, we must approximate the expectation over batches of samples $z_{2}...z_{k}$ from $q(z|x)$. The following are properties of $q_{IW}$:}


% Since this integral is intractable, $q_{IW}$ must be approximated by a finite number of batches.
% below is the distribution

\vspace{-3mm}
\begin{itemize}[leftmargin=5in]
    \item When $k=1$, $q_{IW}(z|x)$ will be equal to $q(z|x)$.
    \item When $k > 1$, the form of $q_{IW}$ depends on the true posterior $p(z|x)$. 
    \item When $k=\infty$, $q_{IW}(z|x)$ becomes the true posterior $p(z|x)$.
\end{itemize}

% Below, we visualize the approximation of $q_{IW}$ with a Gaussian base distribution to a more complex posterior distribution. 

% Below is a visualization of $q_{IW}$ approximating a complex distribution.

Below, we visualize the approximation of $q_{IW}$ to the true posterior with varying number of samples $k$. 

% Below is a visualization of $q_{IW}$ with varying number of samples $k$ and a Gaussian base distribution. 

% the approximation to a complex posterior with varying number of samples k:

% Figure 1 visualizesqIWon a 2D distribution approximation problem.  The base distributionqis aGaussian. As we increase the number of sampleskused for the sampling-resampling, the approxi-mation approaches the true distribution.  This distribution is nonparametric in the sense that, as thetrue posterior grows more complex, so does the shape ofqIW
...
\vspace{5mm}
\normalsize{\qquad \qquad \qquad \qquad Real \qquad \qquad \qquad Sample $q(z|x)$  \qquad \qquad \qquad \qquad \qquad Sample $q_{IW}(z|x)$}
\vspace{-3mm}
\begin{figure}[h]
  \centering
      \includegraphics[width=12in, clip, trim=0cm 0cm 0cm 0cm]{recon_samplingiwae.eps}
...
% The intuition here is that we trained the model with $q_{IW}$ with $K=50$ then sampled from $q(z|x)$ ($q_{IW}$ with $K=1$).
\end{textblock}
