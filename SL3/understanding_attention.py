...
class Att(nn.Module):
    ...
    def multi_head_attention(self, x):
        ...
        print (k.shape, 'after first linear layer in MHA, D1 -> heads*D2, basically this is encoding step')
        print ('reminder that heads get their own attention, so each head can focus on what they want')
        print ('we 3 of the above vectors, k,q,v')
        k = k.transpose(1,2)
        q = q.transpose(1,2)
        v = v.transpose(1,2) #[B,H,L,D/H]
        print (k.shape, 'swap head and length dims')


        values = self.attention(q,k,v, d_k=self.d_k//n_heads)

        print (values.shape, 'after attention func\n')
        

        # concatenate heads and put through final linear layer
        concat = values.transpose(1,2).contiguous().view(B, n, self.d_k)
        print (concat.shape, 'concat heads')
        
        output = self.predict_output(concat)
        print (output.shape, 'put through pred net')
        fasda
    ...
    def attention(self, q, k, v, d_k, mask=None, dropout=None):
        print ('\nAttention with q,k,v of shape', q.shape)
        ...
        print (scores.shape, 'q and v matmul, ie similarity, so it scales with length of sequence squared, because comparing each step with every other')


        mask = self.nopeak_mask
        print (mask.shape, 'mask, its lower triangular, so half similarity computation is wasted')
        print(mask)


        scores = scores.masked_fill(mask == 0, -1e9)  # seems min is that negative value?? its a very negative number.
        print (scores.shape, 'similarities post-mask')
        print (scores[0][0])


        scores = F.softmax(scores, dim=-1)  #[B,H,D,D]
        print (scores.shape, 'similarities post-softmax')
        print (scores[0][0])

        
        output = torch.matmul(scores, v)  #[B,H,D,D]
        print (scores.shape, 'output is similarites matmul with values v')

        print ('\nTo sumamrize MHA, encode each timestep into heads*3*dim, 3 is for q,k,v\n '
                   +'dot product q and v of each timestep, resulting in TxT matrix \n'
                   +'mask, lower triangular, then softmax so ___\n'
                   +'dot with v , then linear layer to output\n')

        print ('wait so is it depdendent on the right timesteps? do I need to shift input?')
        print ('YES because mask makes first thing dependent on 1 thing, thus need first thing to be no info')
        print ()
        print ('woudldnt softmax after mask make it non-zero?? I dont get that ')
        print ('softmax has to go second since things need to sum to 1')
        print ('so it seems that its not completely indepdent, since softmax cant go to zero, but practicaly yes ')
        print()
        print ('how to not waste computation with mask?')
        print ()
...
if train_:
    ...
    for step in range(total_steps):
        ...
        print()
        print(batch_withposition.shape, 'start shape')
        print ()
        # fasda
