...
class Att(nn.Module):
    ...
    def multi_head_attention(self, x):
        ...
        k = self.k_linear(x).view(B, L, n_heads, self.d_k//n_heads)
        q = self.q_linear(x).view(B, L, n_heads, self.d_k//n_heads) #dont query for first position
        v = self.v_linear(x).view(B, L, n_heads, self.d_k//n_heads)
        k = k.transpose(1,2)
        q = q.transpose(1,2)
        ...
        concat = values.transpose(1,2).contiguous().view(B, L, self.d_k)
...
if train_:
    ...
    for step in range(total_steps):
        ...
        for i in range(B):
            ...
            #sample irregularly spaced itervals. 
            ts = [0]
            for i in range(n-1):
                t = ts[-1] + np.random.rand()
                ts.append(t)
            ts = np.array(ts)

            seq = f(ts) + np.random.randn(n)*noise #+ np.random.rand(n) *.1  + np.random.rand(n)
        ...
        batch_t = torch.from_numpy(np.array(batch_t)).float().cuda()


        #shift right 
        batch_shifted = torch.cat([torch.zeros(B,1).cuda(),batch],dim=1)
        batch_shifted = batch_shifted[:,:-1]
        batch_shifted = torch.unsqueeze(batch_shifted, dim=2)  #[B,T,1]

        batch_t_shifted = torch.cat([torch.zeros(B,1).cuda(),batch_t],dim=1)
        batch_t_shifted = batch_t_shifted[:,:-1]
        batch_t_shifted = torch.unsqueeze(batch_t_shifted, dim=2)

        batch_t_shifted = batch_t_shifted / n


        # batch_withposition = concat_position(batch_shifted)

        x = torch.cat([batch_shifted, batch_t_shifted], 2)
    ...
    for plot_i in range (3):
        ...
        for ii in range(1):  
            ...
            for i in range(n-1):
                ...
                ts.append(t)
            ts = np.array(ts)
            seq = f(ts) + np.random.randn(n)*noise
            ...
            # ax.plot(ts, seq, 'b-', label='True', linewidth=1., alpha=.3)
            ax.scatter(ts, seq)
        ...
        ts1 = [ts]
        B=1
        batch = torch.from_numpy(np.array(seq1)).float().cuda()
        batch_t = torch.from_numpy(np.array(ts1)).float().cuda()
        #shift right 
        batch_shifted = torch.cat([torch.zeros(B,1).cuda(),batch],dim=1)
        batch_shifted = batch_shifted[:,:-1]
        batch_shifted = torch.unsqueeze(batch_shifted, dim=2)  #[B,T,1]
        ...
        # ax.plot(ts, seq, 'b-', label='True', linewidth=1.)
        ax.plot(ts, mean, 'g-', label='Pred', linewidth=1.)
        plt.gca().fill_between(ts.flat, mean-std, mean+std, color="#dddddd")

        ax.scatter(ts, seq, alpha=.3)
        ...
        for ii in range(10):
            ...
            # print (x.shape)
            current_xs = x[:,:n//2] #[1,T/2,2]
            print (current_xs.shape)
            # fasdf
            ...
            for t in range(n//2,n):
                ...
                preds = model.forward(current_xs)  #[1,L,2]

                mean = preds[:,-1,0].data.cpu().numpy()
                logvar = preds[:,-1,1].data.cpu().numpy()

                samp = np.random.randn()* np.exp(.5*logvar) + mean
                t = current_xs[0][-1][1].data.cpu().numpy()
                # print (t)


                # # print (preds[].shape)
                # sfasdf
                # mean = torch.unsqueeze(preds[:,-1,0], dim=2)#.data.cpu().numpy()[0][t]
                # logvar = torch.unsqueeze(preds[:,-1,1], dim=2).data.cpu().numpy()[0][t]
