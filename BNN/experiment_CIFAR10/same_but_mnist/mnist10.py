...
class BNN_full(object):
    ...
    def __init__(self, n_classes):
        ...
        # B = tf.shape(self.x)[0]
        # out = tf.reshape(self.x, [B, 28,28,1])
        # conv1_weights = tf.Variable(tf.truncated_normal([5, 5, 1, 20], stddev=0.1))
        ...
        # self.reg =  self.one_over_N*(-log_p_W_sum*.000000001 + log_q_W_sum) 
        self.reg =  10. *(-log_p_W_sum*.000000001 + log_q_W_sum) 
    ...
    def train(self, train_x, train_y, valid_x=[], valid_y=[], display_step=5, 
        ...
        for epoch in range(1,epochs+1):
            ...
            for step in range(n_datapoints/batch_size):
                ...
                if step % display_step  == 0 and epoch %100 == 0:
...
if __name__ == '__main__':
    ...
    path_to_save_variables=home+'/Documents/tmp/vars_mnist.ckpt'
    ...
    if train == 1:
        ...
        model.train(train_x=data_x, train_y=data_y, 
                    epochs=4000, batch_size=50, display_step=3000,
