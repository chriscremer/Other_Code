...
if __name__ == '__main__':
    ...
    # path_to_load_variables=home+'/Documents/tmp/vars_mnist_NN.ckpt'
    path_to_load_variables=''
    # path_to_save_variables=home+'/Documents/tmp/vars_5_classes_relu.ckpt'
    path_to_save_variables=home+'/Documents/tmp/vars_mnist_NN_2.ckpt'
    ...
    if train == 1:
        ...
        model.train(train_x=data_x, train_y=data_y, 
                    epochs=3000, batch_size=50, display_step=3000,
        ...
        print acc
        acc = model.sess.run((model.acc), feed_dict={model.x: test_x, model.y: test_y})
        print acc
        acc = model.sess.run((model.softmax_error), feed_dict={model.x: data_x, model.y: data_y})
        print acc
        acc = model.sess.run((model.softmax_error), feed_dict={model.x: test_x, model.y: test_y})
        print acc
