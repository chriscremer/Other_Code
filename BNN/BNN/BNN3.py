...
#onyl one particle
#now remove logq, so pW should just be like regularizatio

# I removed both p and q and still didnt work. umm so whats the dif of that and NN?
	# dif is the sampling..
	# so w = w_means
	# still no...


# found it! well part of it. Its the initilization on the previous trained BNN
# when I start from random, it works. 


#next model: add in p and q make it not fall into local minima
