...
# from MNF import MNF as BNN
from BNN_aux_var import BNN
        ...
        for epoch in range(1,epochs+1):
            ...
            for step in range(n_datapoints/batch_size):
                ...
                if step % display_step == 0:
                    ...
                    cost, se, p, q = self.sess.run((self.cost, self.softmax_error, self.log_p_W_sum, self.log_q_W_sum), feed_dict={self.x: batch, self.y: batch_y, 
                                                        self.one_over_N: one_over_N})



                    # cost = -cost #because I want to see the NLL
                    print "Epoch", str(epoch)+'/'+str(epochs), 'Step:%04d' % (step+1) +'/'+ str(n_datapoints/batch_size), "cost=", "{:.4f}".format(float(cost)), se, p, q#,logpy,logpW,logqW #, 'time', time.time() - start
...
if __name__ == '__main__':
    ...
    path_to_save_variables=home+'/Documents/tmp/vars_5_classes_aux_2.ckpt'
    ...
    if train == 1:
        ...
        model.train(train_x=new_x, train_y=new_y, 
                    epochs=5, batch_size=50, display_step=3000,
    ...
    if viz:
        ...
        # landscape of likelihood
        # sum of error of train set, for a class, given encoding = z

        z_mean =  model.sess.run((model.second_half_BNN.z_mean))
        print z_mean
        z_logvar =  model.sess.run((model.second_half_BNN.z_logvar))
        print z_logvar
        ...
        likes = []
        costs = []
        for z_i in range(len(flat)):
            print z_i, len(flat)
            likel, cost = model.sess.run((model.softmax_error, model.cost2), feed_dict={model.x: new_x, model.y: new_y, model.second_half_BNN.z: [flat[z_i]]})
            likes.append(-likel)
            costs.append(-cost)

        likes = np.array(likes)
        likes = likes.reshape(X.shape)

        costs = np.array(costs)
        costs = costs.reshape(X.shape)

        ax = plt.subplot2grid((1,3), (0,0), frameon=False)
        cs = ax.contourf(X, Y, likes, cmap='Blues')
        nm, lbl = cs.legend_elements()
        plt.legend(nm, lbl, fontsize=4) 

        ax.scatter([z_mean[0][0]], [z_mean[0][1]], c='red', marker='x')
        ax.scatter([z_mean[0][0]+np.sqrt(np.exp(z_logvar[0][0]))], [z_mean[0][1]], c='black', marker='x')
        ax.scatter([z_mean[0][0]-np.sqrt(np.exp(z_logvar[0][0]))], [z_mean[0][1]], c='black', marker='x')
        ax.scatter([z_mean[0][0]], [z_mean[0][1]+np.sqrt(np.exp(z_logvar[0][1]))], c='black', marker='x')
        ax.scatter([z_mean[0][0]], [z_mean[0][1]-np.sqrt(np.exp(z_logvar[0][1]))], c='black', marker='x')
        ...
        ax = plt.subplot2grid((1,3), (0,1), frameon=False)
        cs = ax.contourf(X, Y, costs, cmap='Blues')
        nm, lbl = cs.legend_elements()
        plt.legend(nm, lbl, fontsize=4) 
        ...
        for z_i in range(len(flat)):
            print z_i, len(flat)
            likel = -model.sess.run((model.softmax_error), feed_dict={model.x: new_x_test, model.y: new_y_test, model.second_half_BNN.z: [flat[z_i]]})
            likes.append(likel)

        likes = np.array(likes)
        likes = likes.reshape(X.shape)

        ax = plt.subplot2grid((1,3), (0,2), frameon=False)
        cs = ax.contourf(X, Y, likes, cmap='Blues')
        nm, lbl = cs.legend_elements()
        plt.legend(nm, lbl, fontsize=4) 
        ...
        n_viz_samples = 3 
        rows=3 #one for max and one for argmax, one for landscape
