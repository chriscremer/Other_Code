...
if __name__ == '__main__':
    ...
    # #same on test set, doesnt matter, all held out
    # new_x_testo = []
    # new_y_testo = []
    # for i in range(len(train_y)):
    ...
    if thign:
        ...
        #calc model vs output uncertainty. 

        #for every training point, calc prediction with mean and with EV
            #calc entropy of those predictions
            #take average over set
        # do for test and held out

        preds = []
        for i in range(20):
            pred = model.sess.run((model.prediction), feed_dict={model.x: new_x})
            preds.append(pred)

        preds = np.array(preds)
        preds = np.mean(preds, axis=0)
        entropy = np.mean(-np.sum(preds*np.log(np.maximum(preds, np.exp(-30))), axis=1))

        print 'Train EV entropy', entropy


        preds = model.sess.run((model.prediction_using_mean), feed_dict={model.x: new_x})
        entropy = np.mean(-np.sum(preds*np.log(np.maximum(preds, np.exp(-30))), axis=1))

        print 'Train mean entropy', entropy



        preds = []
        for i in range(20):
            pred = model.sess.run((model.prediction), feed_dict={model.x: new_x_test})
            preds.append(pred)
        ...
        print 'Test EV entropy', entropy


        preds = model.sess.run((model.prediction_using_mean), feed_dict={model.x: new_x_test})
        entropy = np.mean(-np.sum(preds*np.log(np.maximum(preds, np.exp(-30))), axis=1))

        print 'Test mean entropy', entropy
        ...
        preds = []
        for i in range(20):
            pred = model.sess.run((model.prediction), feed_dict={model.x: new_xo})
            preds.append(pred)
        ...
        print 'Held out EV entropy', entropy


        preds = model.sess.run((model.prediction_using_mean), feed_dict={model.x: new_xo})
        entropy = np.mean(-np.sum(preds*np.log(np.maximum(preds, np.exp(-30))), axis=1))

        print 'Held out mean entropy', entropy
    ...
    if viz:
        ...
        n_viz_samples = 3 
        rows=3 #one for max and one for argmax
        ...
        for j in range(n_viz_samples):
            ...
            predictions_entropy = -np.sum(predictions*np.log(np.maximum(predictions, np.exp(-30))), axis=1) #[N]
            ...
            predictions_entropy= predictions_entropy.reshape(X.shape)
            ...
            ax = plt.subplot2grid((rows,columns), (2,j), frameon=False)#, colspan=3)
            cs_max = ax.contourf(X, Y, predictions_entropy)
            plt.gca().set_aspect('equal', adjustable='box')
            ax.tick_params(labelsize=6)
            if j==0:
                nm, lbl = cs_max.legend_elements()
        ...
        ax = plt.subplot2grid((rows,columns), (2,columns-1), frameon=False)#, colspan=3)

        predictions_entropy = -np.sum(save_predictions_mean*np.log(np.maximum(save_predictions_mean, np.exp(-30))), axis=1) #[N]
        predictions = predictions_entropy.reshape(X.shape)
        ...
        predictions_entropy = -np.sum(predictions*np.log(np.maximum(predictions, np.exp(-30))), axis=1) #[N]
        predictions_entropy = predictions_entropy.reshape(X.shape) #  [numticks, numticks]
        ...
        ax = plt.subplot2grid((rows,columns), (2,columns-2), frameon=False)#, colspan=3)
        cs = ax.contourf(X, Y, predictions_entropy)
        plt.gca().set_aspect('equal', adjustable='box')
        ax.tick_params(labelsize=6)
