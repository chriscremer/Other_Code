    ...
    def feedforward(self, x, W_list):
        ...
        for layer_i in range(len(net)-1):
            ...
            if self.act_func[layer_i] != None:
                ...
                layer_size = tf.shape(cur_val)[1]

                first_slice = tf.tanh(tf.slice(cur_val, [0, 0], [B, layer_size/3]))
                second_slice = tf.nn.softplus(tf.slice(cur_val, [0, layer_size/3], [B, layer_size/3]))
                third_slice = tf.nn.relu(tf.slice(cur_val, [0, 2*layer_size/3], [B, 1+layer_size/3]))

                cur_val = tf.concat([first_slice, second_slice], axis=1)
                cur_val = tf.concat([cur_val, third_slice], axis=1)
