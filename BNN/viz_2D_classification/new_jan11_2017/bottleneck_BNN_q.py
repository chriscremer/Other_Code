...
class bottleneck_BNN(nn.Module):
    ...
    def __init__(self, hyper_config):
        ...
        self.q_logvar = nn.Linear(arch_1[-1][0], arch_1[-1][1])

        # self.add_module('q_logvar', self.q_logvar)
    ...
    def forward(self, x, y):
        ...
        x,logq = self.encode_and_logprob(x)
        y_hat = self.decode(x)
        loss = self.loss(y_hat, y) #[1]

        return loss + .1*logq #+ torch.mean(.1*(self.encode(x))**2)
