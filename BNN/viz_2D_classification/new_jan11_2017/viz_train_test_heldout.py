...
if __name__ == '__main__':
    ...
    if viz_:
        ...
        if view_encodings_:
            ...
            train_samples_from_each = [[] for x in range(5)]
            for i in range(len(train_y)):
                if len(train_samples_from_each[train_y[i]]) < n:
                    train_samples_from_each[train_y[i]].append(train_x[i])   
                    # if np.array([len(x) for x in samples_from_each]).all():
                    #     break

            test_samples_from_each = [[] for x in range(5)]
            for i in range(len(test_y)):
                if test_y[i] < 5:
                    if len(train_samples_from_each[test_y[i]]) < n:
                        train_samples_from_each[test_y[i]].append(test_x[i]) 


            heldout_samples_from_each = [[] for x in range(5)]
            for i in range(len(train_y)):
                if train_y[i] >= 5:
                    if len(heldout_samples_from_each[train_y[i]-5]) < n:
                        heldout_samples_from_each[train_y[i]-5].append(train_x[i])
        ...
        train_samples_from_each_encoded = []
        for i in range(5):
            pytorch_input = Variable(torch.from_numpy(np.array(train_samples_from_each[i])))
            # encodings_ = model.encode(pytorch_input).data.numpy() #[n,2]
            encodings_ = model.encode(pytorch_input).data.numpy() #[n,2]
            train_samples_from_each_encoded.append(encodings_)

        #See scatter
        for c in range(5):
                aaa = ax.scatter(train_samples_from_each_encoded[c].T[0], train_samples_from_each_encoded[c].T[1], alpha=.5, label=str(c), c='black', marker='x', s=1)
                for ii in range(len(train_samples_from_each_encoded[c])):
                    if c < n_limited_classes:
                        plt.text(train_samples_from_each_encoded[c][ii][0], train_samples_from_each_encoded[c][ii][1], str(c), color="black", fontsize=6)
                    else:
        ...
        # for j in range(12):
            #convert to encodings
        heldout_samples_from_each_encoded = []
        for i in range(5):
            pytorch_input = Variable(torch.from_numpy(np.array(heldout_samples_from_each[i])))
            # encodings_ = model.encode(pytorch_input).data.numpy() #[n,2]
            encodings_ = model.encode(pytorch_input).data.numpy() #[n,2]
            heldout_samples_from_each_encoded.append(encodings_)

        #See scatter
        for c in range(5):
                aaa = ax.scatter(heldout_samples_from_each_encoded[c].T[0], heldout_samples_from_each_encoded[c].T[1], alpha=.5, label=str(c), c='black', marker='x', s=1)
                for ii in range(len(heldout_samples_from_each_encoded[c])):
                    if c < n_limited_classes:
                        plt.text(heldout_samples_from_each_encoded[c][ii][0], heldout_samples_from_each_encoded[c][ii][1], str(c), color="black", fontsize=6)
                    else:
                        # plt.text(samples_from_each_encoded[c][ii][0], samples_from_each_encoded[c][ii][1], str(c), color="red", fontsize=6)
                        continue
