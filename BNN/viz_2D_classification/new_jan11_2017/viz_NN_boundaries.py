...
if __name__ == '__main__':
    ...
    if viz_:
        ...
        if view_encodings_:
            ...
            #         if len(samples_from_each[original_train_y[i]]) < n:
            #             samples_from_each[original_train_y[i]].append(train_x[i])
            #             # if np.array([len(x) for x in samples_from_each]).all():
            #             #     break
            ...
            for i in range(n_all_classes):
                ...
                # encodings_ = model.feedforward(pytorch_input).data.numpy() #[n,2]
                encodings_ = model.encode(pytorch_input).data.numpy() #[n,2]
                # print (encodings_.shape)
                # fadfs
        ...
        for c in range(n_all_classes):
            ...
            for ii in range(len(samples_from_each_encoded[c])):
                if c < n_limited_classes:
                    print (c)
                    print (samples_from_each_encoded[c][ii][0], samples_from_each_encoded[c][ii][1])
