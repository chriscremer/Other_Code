...
def log_normal4(x, mean, log_var):
    ...
    D = tf.to_float(tf.shape(mean)[1])
    term1 = D * tf.log(2*math.pi) #[1]
    term2 = tf.reduce_sum(log_var, axis=1) #sum over D, [PB]
    dif_cov = tf.square(x - mean) / tf.exp(log_var) #[PB,D]
    term3 = tf.reduce_sum(dif_cov, axis=1) #sum over D, [PB]
    all_ = term1 + term2 + term3  #[PB]
    log_N = -.5 * all_
    return log_N
