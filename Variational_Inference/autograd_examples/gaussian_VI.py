...
def init_gaussian_var_params(D, mean_mean=-1, log_std_mean=-5,
                             scale=0.1, rs=npr.RandomState(np.random.randint(0,6))):
...
def build_mog_bbsvi(logprob, num_samples, k=10, rs=npr.RandomState(np.random.randint(0,20))):
    ...
    def init_var_params(D, rs=npr.RandomState(np.random.randint(0,6)), **kwargs):
    ...
    def softmax(x):
        ...
        return e_x / np.sum(e_x) #.sum(axis=0)
...
if __name__ == '__main__':
    ...
    def plot_isocontours(ax, func, xlimits=[-5, 5], ylimits=[-8, 4],
