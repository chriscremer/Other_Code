...
class Factorized_Gaussian_model(object):
    ...
    def __init__(self, log_posterior):
        ...
        self.qz_mean = tf.Variable([0.,0.])
        self.qz_logvar = tf.Variable([.1,.1])
        ...
        self.log_q_z = log_normal_eps(eps, self.qz_logvar)
        ...
        self.grad_mean = tf.gradients(self.elbo, self.qz_mean)
        self.grad_logvar = tf.gradients(self.elbo, self.qz_logvar)
...
class AV_model(Factorized_Gaussian_model):
    ...
    def __init__(self, log_posterior):
        ...
        self.elbo = self.log_p_z  - self.log_q_z +  self.log_r_v - self.log_q_v
