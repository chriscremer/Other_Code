...
def test(model, path_to_load_variables='', batch_size=300, display_epoch=5000, k=10):
...
# variables['mean_m'] = (variables['mean1']*variables['linear_transform']) + variables['bias_transform']
# variables['logvar_m'] = torch.log(torch.exp(model.logvar2) + model.linear_transform.pow(2)*torch.exp(model.logvar1))
...
class IWG(nn.Module):
    ...
    def __init__(self, dim, logpz):
        ...
        self.mean = Variable(torch.Tensor(variables['mean1']), requires_grad=True)
        self.logvar = Variable(torch.Tensor(variables['logvar1']), requires_grad=True)
...
train(model=model, 
            ...
            epochs=1000, batch_size=4, display_epoch=40, k=2)
