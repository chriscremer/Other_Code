...
def plot_hi_isocontours(ax, hamiltonian_sampling, k=3, xlimits=[-6, 6], ylimits=[-6, 6], numticks=101, 
                        n_sample_batches=3, cmap='Blues'):
    ...
    for b in range(n_sample_batches):
        ...
        z0, zT, v0, vT, log_q_z0, log_q_zT, log_q_v0, log_q_vT  = hamiltonian_sampling(k)
...
def hamiltonian_sampling(n_samples, target_dist, proposal_dist, proposal_sampler):
    ...
    return z_0, z, v_0, v, log_q_z0, log_q_zT, log_q_v0, log_q_vT




def hamiltonian_sampling_debug(n_samples, target_dist, proposal_dist, proposal_sampler):
...
if __name__ == '__main__':
    ...
    #debug
    z0, zT, v0, vT, log_q_z0, log_q_zT, log_q_v0, log_q_vT = sess.run(hamiltonian_sampling_debug(n_samples=tf.convert_to_tensor(100, dtype='int32'), 
    ...
    hamil_func = lambda x: sess.run(hamiltonian_sampling(n_samples=tf.convert_to_tensor(x, dtype='int32'), 
                                    ...
                                    proposal_dist=proposal, 
                                    proposal_sampler=sample_proposal))
    plot_hi_isocontours(ax, hamil_func, k=200, n_sample_batches=1, cmap='Reds')
    

    z0, zT, v0, vT, log_q_z0, log_q_zT, log_q_v0, log_q_vT = sess.run(hamiltonian_sampling(n_samples=tf.convert_to_tensor(5, dtype='int32'), 
                                    target_dist=lambda y: log_posteriors(0,y), 
                                    proposal_dist=proposal, 
                                    proposal_sampler=sample_proposal))
