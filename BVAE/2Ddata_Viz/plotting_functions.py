...
def plot_isocontours(ax, func, xlimits=[-6, 6], ylimits=[-6, 6],
    ...
    X, Y = np.meshgrid(x, y)
    # print np.concatenate([np.atleast_2d(X.ravel()), np.atleast_2d(Y.ravel())]).T.shape
    # fdsaf
...
def return_z_values(func, xlimits=[-6, 6], ylimits=[-6, 6],
                     numticks=101, cmap=None, alpha=1.):
    x = np.linspace(*xlimits, num=numticks)
    y = np.linspace(*ylimits, num=numticks)
    X, Y = np.meshgrid(x, y)
...
def plot_isocontours_mog(ax, gaus_class, means, logvars, xlimits=[-6, 6], ylimits=[-6, 6],
                     numticks=101, cmap=None, alpha=1.):
    ...
    def func(x):
        ...
        for i in range(len(means)):
            px_z = gaus_class(means[i], logvars[i])
            prob = np.exp(px_z.run_log_post(x))
            probs.append(prob)
...
def plot_scatter(ax, samps, color='blue', alpha=1., s=10):# dist, n_samps):

    # samps = [dist.run_sample_post() for x in range(n_samps)]
    # samps = np.array(samps)

    # plt.scatter(z0.T[0], z0.T[1], color='yellow', marker='x')
    plt.scatter(samps.T[0], samps.T[1], color=color, marker='x', zorder=2, alpha=alpha, s=s)
    ax.set_yticks([])
    ax.set_xticks([])
    plt.xlim(-6, 6)
    plt.ylim(-6, 6)
