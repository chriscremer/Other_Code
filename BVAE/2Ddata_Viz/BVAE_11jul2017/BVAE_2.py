...
from utils import log_normal4 as log_norm4
...
class BVAE(object):
    ...
    def __init__(self, hyperparams):
        ...
        rec_hidden_layers = hyperparams['encoder_hidden_layers']
        gen_hidden_layers = hyperparams['decoder_hidden_layers']

        if self.likelihood == 'Bernoulli':
            decoder_output_size = self.x_size
        elif self.likelihood == 'Gaussian':
            decoder_output_size = self.x_size*2

        if hyperparams['ga'] == 'none':
            self.encoder_net = [self.x_size] + rec_hidden_layers + [self.z_size*2]

        elif hyperparams['ga'] == 'hypo_net':
            #count size of decoder 
            if len(gen_hidden_layers) == 0:
                n_decoder_weights = (self.z_size+1) * self.x_size
            else:
                n_decoder_weights = 0
                for i in range(len(gen_hidden_layers)+1):
                    if i==0:
                        n_decoder_weights += (self.z_size+1) * gen_hidden_layers[i]
                    elif i==len(gen_hidden_layers):
                        n_decoder_weights += (gen_hidden_layers[i-1]+1) * decoder_output_size
                    else:
        ...
        with tf.variable_scope("log_probs"):
            ...
            if hyperparams['logprob_type'] == 2:
                log_probs = self.log_probs2(self.x, encoder, decoder, sample_z)
    ...
    def log_probs(self, x, encoder, decoder, sample_z):
        ...
        log_pW0 = tf.zeros([1]) 
        log_qW0 = tf.zeros([1]) 
        # means = tf.zeros([1,self.n_z_particles, self.batch_size, self.x_size]) 
        # logvars = tf.zeros([1,self.n_z_particles, self.batch_size, self.x_size]) 
    ...
    def objective(self, log_px, log_pz, log_qz, log_pW, log_qW):
        ...
        elbo = (self.log_px + self.log_pz - self.log_qz 
                + self.batch_frac*((self.log_pW*self.pW_weight) - (self.log_qW*self.qW_weight)) 
    ...
    def init_params(self, path_to_load_variables):
    ...
    def sample_prior(self, path_to_load_variables, n_samps, need_to_init=False):
        ...
        eps = np.random.normal(0., 1., n_samps*self.z_size)
        eps = np.reshape(eps, [n_samps, self.z_size])


        means, logvars  = self.sess.run((self.x_mean, self.x_logvar), feed_dict={self.z: eps})
    ...
    def generate(self, z):
        ...
        means, logvars  = self.sess.run((self.x_mean, self.x_logvar), feed_dict={self.z: z})
    ...
    def reconstruct(self, path_to_load_variables, samps, need_to_init=False):
        ...
        means, logvars  = self.sess.run((self.x_mean, self.x_logvar), feed_dict={self.x: samps})
        ...
        with tf.Session() as self.sess:
            ...
            for epoch in range(1,epochs+1):
                ...
                for step in range(n_datapoints/batch_size):
                    ...
                    # Display logs per epoch step
                    # if step % display_step[1] == 0 and epoch % display_step[0] == 0:
                    if (epoch % display_step == 0 or epoch == 1) and step ==0:
                        ...
                        print ("Epoch", str(epoch)+'/'+str(epochs), 
                                'Step:%04d' % (step) +'/'+ str(n_datapoints/batch_size), 
                                "elbo={:.4f}".format(float(elbo)),
                                '{:.2f}'.format(float(logpx)),
                                '{:.2f}'.format(float(logpz)),
                                '{:.2f}'.format(float(logqz)),
                                '{:.2f}'.format(float(logpW)),
                                '{:.2f}'.format(float(logqW)),
                                '{:.2f}'.format(float(l2_sum)))
