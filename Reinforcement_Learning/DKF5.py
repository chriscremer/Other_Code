...
# same as 4 but replace map over particles with a for loop
# doing this because of the bug 

#this has no backprop....

# so changing the function to outuptu the probs with the particles.
...
class DKF():
    ...
    def Model(self, x, actions, network_weights):
        ...
        def fn_over_timesteps_for_scan(prev_output, xap_i):
            '''
            prev_output: [particles,logpx,logpz,logqz]
            xap_i: [B,X+A+X]
            '''
            ...
            z = tf.slice(prev_output, [0,0,0],[self.n_particles, self.batch_size, self.z_size])
            ...
            for k in range(self.n_particles):
                ...
                zi = tf.slice(z, [0, k, 0], [self.batch_size, 1, self.z_size])
                #reshape [B,Z]
                zi = tf.reshape(zi, [self.batch_size, self.z_size])
            ...
            # [3,B]
            logprobs_init = tf.pack([px_init, pz_init, qz_init])
            # do this to make it one tensor
            logprobs_init = tf.reshape(logprobs_init, [3,B,1])
            # [3,B,Z]
            logprobs_init = tf.tile(logprobs_init, [1,1,self.z_size])
            # [P+3, B, Z]
            initializer = tf.concat(0, [z_init, logprobs_init])

            #return the new zs, which will be prev_z for the next iteration
            # print (tf.pack(particles), log_p_x_, log_p_z_, log_q_z_)
            return initializer
        ...
        logprobs_init = tf.pack([px_init, pz_init, qz_init])
        # do this to make it one tensor
        logprobs_init = tf.reshape(logprobs_init, [3,self.batch_size,1])
        # [3,B,Z]
        logprobs_init = tf.tile(logprobs_init, [1,1,self.z_size])
        # [P+3, B, Z]
        initializer = tf.concat(0, [z_init, logprobs_init])
