...
class Univariate_Gaussian_SVI():
    ...
    def __init__(self, learning_rate=0.001, n_particles=1):
        ...
        self.log_pi1 = tf.Variable([.5])

        self.mean2 = tf.Variable([-1.])
        self.log_var2 = tf.Variable([1.])
        self.log_pi2 = tf.Variable([.5])

        self.pi1 = tf.exp(self.log_pi1) / (tf.exp(self.log_pi1) + tf.exp(self.log_pi2))
        self.pi2 = tf.exp(self.log_pi2) / (tf.exp(self.log_pi1) + tf.exp(self.log_pi2))
