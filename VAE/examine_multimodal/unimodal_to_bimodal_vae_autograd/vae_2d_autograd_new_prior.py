...
def generate_from_prior(gen_params, num_samples, num_latent_dims): #, rs):
    ...
    aaa = np.random.randint(0,2)
    if aaa ==0:
        latents = np.exp(.1)*npr.randn(num_samples, num_latent_dims) + 1
    if aaa ==1:
        latents = np.exp(.1)*npr.randn(num_samples, num_latent_dims) - 1
...
def vae_lower_bound(gen_params, rec_params, data, rs):
    ...
    p_latents1 = diag_gaussian_log_density(latents, -1, .1)
    p_latents2 = diag_gaussian_log_density(latents, 1, .1)
    p_latents = np.log(.5 * np.exp(p_latents1) + .5 * np.exp(p_latents2))
...
if __name__ == '__main__':
    ...
    plt.plot([],[],label=str(latent_dim)+' latent dims. MoG Prior')
