...
def train(model, train_x, train_y, valid_x=[], valid_y=[], 
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            # # g = make_dot(loss, model.parameters())
            # g.view()
            # fasdf
            ...
            if epoch%display_epoch==0 and batch_idx == 0:
                ...
                # image = model.decode(Variable(torch.zeros(1,model.z_size)))

                images = model.x_hat_sigmoid

                # print images.data.shape
                # fsdfas
                image = images.data.numpy()[0]

                plt.imshow(image.reshape(28, 28), cmap='gray')
                # plt.show(block=True)d
