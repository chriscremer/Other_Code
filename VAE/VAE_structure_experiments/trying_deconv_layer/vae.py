...
# this is a vae set up like a bvae, so that I can compare them easily
...
# from NN import NN
from conv_NN import conv_NN as NN

# from NN_like_BNN import NN as BNN
from conv_NN_like_BNN import conv_NN as BNN
...
class VAE(object):
    ...
    def __init__(self, hyperparams):
        ...
        intermediate_layer_size = 180

        encoder_convnet = [filter_height, filter_width, n_in_channels, n_out_channels, strides, intermediate_layer_size, hidden_layer_size]
        decoder_deconvnet = [filter_height, filter_width, n_in_channels, n_out_channels, strides, intermediate_layer_size, hidden_layer_size]

        #Define endocer and decoder
        with tf.variable_scope("encoder"):
            # encoder = NN(self.encoder_net, self.encoder_act_func, self.batch_size)
            encoder = NN(self.x_size, self.z_size, encoder_convnet)


        with tf.variable_scope("decoder"):
            # decoder = BNN(self.decoder_net, self.decoder_act_func, self.batch_size)
            decoder = BNN(self.x_size, self.z_size, decoder_deconvnet)
...
if __name__ == '__main__':
    ...
    model.train(train_x=train_x,
                epochs=epochs, batch_size=batch_size, display_step=[1,1000],
    ...
    print 'Eval'
    iwae_elbo = model.eval(data=test_x, batch_size=batch_size, display_step=10,
