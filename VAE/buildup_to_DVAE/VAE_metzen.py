...
class VariationalAutoencoder(object):
    ...
    def _create_loss_optimizer(self):
        ...
        reconstr_loss = \
            -tf.reduce_sum(self.x * tf.log(1e-10 + self.x_reconstr_mean)
                           ...
                           1)
        self.p1 = tf.reduce_mean(reconstr_loss)
        ...
        self.p2 = tf.reduce_mean(latent_loss)
    ...
    def partial_fit(self, X):
        ...
        """
        opt, cost, p1, p2 = self.sess.run((self.optimizer, self.cost, self.p1, self.p2), 
                                  feed_dict={self.x: X})
        return cost, p1, p2
...
def get_vector_batch(vector_length, n_batch):
...
def train(network_architecture, learning_rate=0.001,
    ...
    for epoch in range(training_epochs):
        ...
        batch_xs = get_vector_batch(vector_length=10, n_batch=batch_size)

        # Fit training using batch data
        cost, p1, p2 = vae.partial_fit(batch_xs)
...
vae = train(network_architecture, training_epochs=1000, batch_size=batch_size, display_step=50)
...
scipy.misc.toimage(generated, cmin=0., cmax=1.).save(save_to +'outfile.jpg')
print 'saved generation to' +save_to +'outfile.jpg'
