...
if __name__ == "__main__":
    ...
    #     #remove time dimension for now
    #     sequence = np.reshape(sequence, [-1])
    #     return sequence

    def get_sequence_1(vector_length):
        ...
        # batch.append(state)
        # noise = np.random.randint(0, 4) /10.
        # state += noise

        # state = state + (.1 * np.abs(np.random.randn(vector_length)))
    ...
    path_to_save_variables=save_to + 'vae_vector2.ckpt'
    ...
    def get_ball():
        return get_sequence(vector_length=n_x)
    ...
    print 'Training'
    sess = vae.train(get_data=get_ball, steps=training_steps, display_step=500, path_to_load_variables=path_to_load_variables, path_to_save_variables=path_to_save_variables)

    print 'Generating'
    generated = vae.generate(load=False, path_to_load_variables='', z_mu=None)
    print 'Reconstructing'
    batch = []
    for i in range(n_batch):
        seq = get_sequence(vector_length=n_x)
        batch.append(seq)
    batch = np.array(batch)
    ...
    scipy.misc.toimage(batch, cmin=0., cmax=1.).save(save_to +'original.jpg')
    scipy.misc.toimage(generated, cmin=0., cmax=1.).save(save_to +'generated.jpg')
    scipy.misc.toimage(reconstructed, cmin=0., cmax=1.).save(save_to +'reconstructed.jpg')
