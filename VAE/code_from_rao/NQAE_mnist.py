...
class NQAE():
    ...
    def __init__(self, network_architecture, transfer_fct=tf.nn.softplus, learning_rate=0.001, batch_size=5, n_particles=3):
        ...
        self.rnn_state_size = network_architecture["rnn_state_size"]
    ...
    def _initialize_weights(self, n_hidden_recog_1, n_hidden_recog_2, n_hidden_recog_3,
                            ...
                            n_input, n_z, rnn_state_size):
        all_weights = dict()
        all_weights['weights_recog'] = {
            'h1': tf.Variable(xavier_init(n_input+rnn_state_size, n_hidden_recog_1)),
            'h2': tf.Variable(xavier_init(n_hidden_recog_1, n_hidden_recog_2)),
            'out_mean': tf.Variable(xavier_init(n_hidden_recog_2, n_z)),
            'out_log_sigma': tf.Variable(xavier_init(n_hidden_recog_2, n_z)),

            'h3': tf.Variable(xavier_init(n_hidden_recog_2, n_hidden_recog_3)),
            'out_state': tf.Variable(xavier_init(n_hidden_recog_3, rnn_state_size))
            }
        all_weights['biases_recog'] = {
            ...
            'out_state': tf.Variable(tf.zeros([rnn_state_size], dtype=tf.float32)),
            }
    ...
    def _recognition_network(self, x_t, rnn_state, weights, biases):
        # Generate probabilistic encoder (recognition network), which
        # maps inputs onto a normal distribution in latent space.

        concatenate_x_and_state = tf.concat(1, [x_t, rnn_state])
        ...
        layer_3 = self.transfer_fct(tf.add(tf.matmul(layer_2, weights['h3']), biases['b3'])) 

        new_state = tf.add(tf.matmul(layer_3, weights['out_state']), biases['out_state'])

        return z_mean_t, z_log_sigma_sq_t, new_state
    ...
    def _log_q_z_given_x(self, z, mean, log_var_sq):
