...
class VAE():
    ...
    def __init__(self, network_architecture, transfer_fct=tf.nn.softplus, learning_rate=0.001, batch_size=5):
    ...
    def _create_loss_optimizer(self):
        ...
        reconstr_loss = \
                tf.reduce_sum(tf.maximum(self.x_reconstr_mean_no_sig, 0) 
                            - self.x_reconstr_mean_no_sig * self.x 
                            + tf.log(1 + tf.exp(-abs(self.x_reconstr_mean_no_sig))),
                             1)
    ...
    def train(self, get_data, steps=1000, display_step=5):
...
if __name__ == "__main__":
    ...
    def get_ball():
        return make_ball_gif(n_frames=1, f_height=f_height, f_width=f_width, ball_size=ball_size, max_1=True, vector_output=True)
    ...
    network_architecture = \
        dict(n_hidden_recog_1=100, # 1st layer encoder neurons
             ...
             n_input=f_height*f_width, # 14 x 14 image
             n_z=20)  # dimensionality of latent space

    vae = VAE(network_architecture, transfer_fct=tf.nn.softplus, learning_rate=0.001, batch_size=5)
    vae.train(get_data=get_ball, steps=5000, display_step=5)
    ...
    for i in range(len(x_reconstruct)):
        ...
        reconstruct_batch[i] = reconstruct_batch[i] * (255. / np.max(reconstruct_batch[i]))
        reconstruct_batch[i] = reconstruct_batch[i].astype('uint8')
    # kargs = { 'duration': .5 }
    ...
    for i in range(len(batch)):
        ...
        combined.append(reconstruct_batch[i])
    ...
    for i in range(len(generated)):
        generated2.append(np.reshape(generated[i], [f_height,f_width,1]))
        generated2[i] = generated2[i] * (255. / np.max(generated2[i]))
        generated2[i] = generated2[i].astype('uint8')

    kargs = { 'duration': .5 }
    imageio.mimsave(home+"/Downloads/gen_gif.gif", generated2, 'GIF', **kargs)
