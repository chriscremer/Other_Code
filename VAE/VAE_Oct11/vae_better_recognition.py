...
class VAE():
    ...
    def _initialize_weights(self, n_hidden_recog_1, n_hidden_recog_2, 
        ...
        all_weights['weights_recog'] = {
            'h1': tf.Variable(xavier_init(n_input+n_z, n_hidden_recog_1)),
    ...
    def _recognition_network(self, weights, biases):
        ...
        prev_mean = tf.zeros([self.batch_size,self.network_architecture["n_z"]])
        for time_step in range(self.n_time_steps):
            ...
            #concat the prev mean
            batch_at_time_step = tf.concat(1, [batch_at_time_step, prev_mean])
    ...
    # def _recognition_network_one_step(self, weights, biases):
    #     # Generate probabilistic encoder (recognition network), which
    #     # maps inputs onto a normal distribution in latent space.
    #     # The transformation is parametrized and can be learned.
    ...
    def _generator_network(self, weights, biases):
        ...
        for time_step in range(self.n_time_steps):
            ...
            #slice: where to start, how many to get
            batch_at_time_step = tf.slice(self.z, [0,time_step,0], [self.batch_size, 1, self.network_architecture["n_z"]])
            batch_at_time_step = tf.reshape(batch_at_time_step, [self.batch_size,self.network_architecture["n_z"]])

            layer_1 = self.transfer_fct(tf.add(tf.matmul(batch_at_time_step, weights['h1']), 
                                               biases['b1'])) 
            layer_2 = self.transfer_fct(tf.add(tf.matmul(layer_1, weights['h2']), 
                                               biases['b2'])) 
    ...
    def _create_loss_optimizer(self):
        ...
        #ALSO train a transition network on the q(z|x), to get me p(z_t|z_t-1)
        trans_loss_list = []

        for time_step in range(self.n_time_steps):

            z_t = tf.slice(self.z_mean, [0,time_step,0], [self.batch_size, 1, self.network_architecture["n_z"]])
            z_t = tf.reshape(z_t, [self.batch_size,self.network_architecture["n_z"]])

            if time_step != 0:  

                #calc error
                z_pred = self._transition_network(prev_z_mean, self.network_weights["weights_trans"], self.network_weights["biases_trans"]) 

                trans_loss = tf.reduce_sum(tf.square(z_pred -  z_t), 1)
                trans_loss_list.append(trans_loss)

            prev_z_mean = z_t


        trans_loss_tensor = tf.pack(trans_loss_list, axis=1)

        #Sum over time steps
        trans_loss_for_each_sequence = tf.reduce_sum(trans_loss_tensor, reduction_indices=1)



        # average over batch
        self.cost = tf.reduce_mean(reconstr_loss_for_each_sequence + latent_loss_for_each_sequence + trans_loss_for_each_sequence)   
    ...
    def generte_sequential_z(self):

        zs = []
        # first_z = np.random.normal(size=(self.batch_size, self.network_architecture["n_z"]))
        first_z = tf.random_normal(shape=(self.batch_size, self.network_architecture["n_z"]))

        zs.append(first_z)
        prev_z = first_z
        for i in range(self.n_time_steps -1 ):
            new_z = self._transition_network(prev_z, self.network_weights["weights_trans"], self.network_weights["biases_trans"])
            zs.append(new_z)
            prev_z = new_z
    ...
    def generate(self, z_mu=None):
        ...
        aaaaa = self.sess.run(self.x_reconstr_mean, 
...
if __name__ == "__main__":
    ...
    combined = []
    for i in range(0,len(batch),n_time_steps):
    ...
    print 'saved ' + home+"/Downloads/comb_gif.gif"



    #GENERATE 

    zs_ = vae.generte_sequential_z().eval()
    print zs_.shape

    generated = vae.generate(z_mu=zs_)
    ...
    print 'saved '+home+"/Downloads/gen_gif.gif"
