...
class VAE():
    ...
    def _recognition_network(self, z_t_minus_1, x_t, weights, biases):
        # Generate probabilistic encoder (recognition network), which
        # maps inputs onto a normal distribution in latent space.

        concatenate_state_and_frame = tf.concat(1, [z_t_minus_1, x_t])

        layer_1 = self.transfer_fct(tf.add(tf.matmul(concatenate_state_and_frame, weights['h1']), biases['b1'])) 
    ...
    def _generative_transition_prior_network(self, prev_z, weights, biases):
    ...
    def _create_loss_optimizer(self):
        ...
        for time_step in range(self.n_time_steps):
            ...
            if time_step == 0:
                ...
                prior_mean, prior_log_sigma_sq = self._generative_transition_prior_network(prev_z, self.network_weights["weights_trans"], self.network_weights["biases_trans"])
    ...
    def call_generate(self):
        return self.sess.run(self.generate_1_)
    ...
    def train(self, get_data, steps=1000, display_step=10, path_to_load_variables='', path_to_save_variables=''):
        ...
        for step in range(steps):
            ...
            cost, rec, kl, nw = vae.partial_fit(batch)
