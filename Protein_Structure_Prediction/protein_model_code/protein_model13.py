...
#actaully, maybe its not needed, because if they want to have higher gating weight,
#then there needds to be distnace , so maybe repel cost just makes it more compliated


#WHat i want is if there is no prediction power then it should be a straight line
...
nu = .1/(L) #repel all
lmbd = nu * (L-1) #adj
rho = .001/(L*n_aa)  #.01 #weight decay
...
def gate():
	...
	dif_matrix = L_by_L - tf.transpose(L_by_L, perm=[0, 2, 1]) + tf.reshape(tf.concat(0,[tf.diag(tf.ones([L]))*100, tf.diag(tf.ones([L]))*100, tf.diag(tf.ones([L]))*100]),[D,L,L])
...
def pull_adj():
	...
	first_D2 = tf.gather(tf.reshape(tf.gather(tf.reshape(column_2D_positions, [D, L]), [2]), [L]), range1)
	...
	second_D2 = tf.gather(tf.reshape(tf.gather(tf.reshape(column_2D_positions, [D, L]), [2]), [L]), range2)


	sum1 = tf.reduce_sum(tf.exp(tf.square(first_D0 - second_D0) + (tf.square(first_D1 - second_D1)) + (tf.square(first_D2 - second_D2))))
...
def repel_all():
	...
	#wait this shouldnt work since some would be 0
	dist_matrix = 1./dist_matrix
	total = tf.reduce_sum(dist_matrix)
...
# square = tf.square(dif_matrix)
# dist_matrix = tf.exp(tf.reduce_sum(square,0))
# dist_matrix1 = 1./dist_matrix
# total = tf.reduce_sum(dist_matrix1)
...
for i in range(50):
	...
	if i%5==0 and (best_error > avg_valid_error or best_error == -1):
		...
		#Plot
		# current_positions = sess.run(column_2D_positions)
		# plt.scatter(current_positions[0], current_positions[1])
		# for i in range(len(current_positions[0])):
		# 	plt.annotate(str(i), xy = (current_positions[0][i], current_positions[1][i]))
		# plt.show()
...
for i in range(len(final_positions)):
	node = []
	for j in range(len(final_positions[i])):
		node.append(float(final_positions[i][j]))
...
# Writing JSON data
with open('../threejs/test1.json', 'w') as f:
	json.dump(data, f)

print 'Sent to json file'

#repel all and adj is broken, the larger the weight gets, the closer they get
#also gets nan when inceraed

# TODO
#get 2d working then make it 3d
