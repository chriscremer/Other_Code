...
repel_cost = mew * repel_all()

cost_function = cross_entropy + weight_decay + adj_cost
...
for i in range(50):
	...
	for samp in range(len(X)):
		...
		if samp < 400:
			# test = sess.run(aaa, feed_dict={input_tensor: convert_samp_to_L_by_L(X[samp]), target: convert_samp_to_one_hot(X[samp])})
