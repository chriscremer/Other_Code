...
for l in range(len_protein):
	...
	def model(input_aa, w, gating_network):
		...
		return tf.nn.softmax(tf.matmul(tf.transpose(gating_network), (tf.reshape(tf.batch_matmul(w, input_aa), [len_protein-1, 22]) + b)))


	aaa = this_aa_position
	bbb = other_aa_positions

	gating_network = tf.reshape(tf.square(other_aa_positions - this_aa_position) / tf.reduce_sum(tf.square(other_aa_positions - this_aa_position)), [len_protein-1,1])
