...
class Structure_Learner():
	...
	def __init__(self, L=None, n_aa=22, tol=2., B=20, lr=.01, mom=.9, nu=.1, rho=.01, sigma_repel=.01, sigma_adj=.1, coordinates=[]):
	...
	def fit(self, X_train, X_valid, experts):
		'''
		...
		prediction = self.feed_forward(input_tensor, gating_network, experts)
		...
		for iiii in range(999999):
			...
			print str(samp) + ' cost: ' + str(cost) + ' CE: ' + str(ce) + ' AC: ' + str(pt) + ' RC: ' + str(rc)
	...
	def feed_forward(self, input1, gating_network, experts):
