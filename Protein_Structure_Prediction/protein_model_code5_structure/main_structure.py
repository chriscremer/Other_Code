...
if __name__ == "__main__":
	...
	mypath = home + '/Documents/Protein_data/RASH/experts'
	for f in listdir(mypath):
		if f.startswith('exp'):
			numba = int(f.split('.')[0].split('_')[1])
			with open(mypath + '/' + f, 'rb') as ff:
				exparte = pickle.load(ff)
				exparte = np.array(exparte)
				# print np.sum(exparte)
				# print exparte[59]
				experts_weights[numba] = exparte

	experts_weights = np.float32(experts_weights)

	# print experts_weights[0]

	print np.sum(experts_weights)
			...
			for rho in rho_list:
				...
				for sigma_repel in sigma_repel_list:
					...
					for sigma_adj in sigma_adj_list:
						...
						model1.fit(X_train, X_valid, experts_weights)
