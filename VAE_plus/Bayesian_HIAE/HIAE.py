...
class HIAE(VAE):
    ...
    def __init__(self, network_architecture, learning_rate=0.0001, batch_size=5, n_particles=3):
        ...
        #Sample
        eps = tf.random_normal((self.n_particles, self.batch_size, self.n_z), 0, 1, dtype=tf.float32)
        self.z_0 = tf.add(self.recog_means, tf.multiply(tf.sqrt(tf.exp(self.recog_log_vars)), eps)) #uses broadcasting, z=[n_parts, n_batches, n_z]

        self.T = 5
        self.friction = .95
        self.step_size = .1

        self.z, self.v_0, self.v_T = self.leapfrogs(self.z_0, self.T, self.friction, self.step_size, self.x)
        
        #Decoder - Generative model - p(x|z)
        self.x_reconstr_mean_no_sigmoid = self._generator_network(self.z, self.network_weights['decoder_weights'], self.network_weights['decoder_biases']) #no sigmoid

        #Objective
        self.elbo = self.elbo(self.x, self.x_reconstr_mean_no_sigmoid, self.z_0, self.recog_means, self.recog_log_vars, self.z, self.v_0, self.v_T)
        ...
        self.log_w = self._log_likelihood(self.x, self.x_reconstr_mean_no_sigmoid) + self._log_p_z(self.z) - self._log_q_z_given_x(self.z_0, self.recog_means, self.recog_log_vars) + (self.T*tf.log(self.friction)) + self._log_p_z(self.v_T) - self._log_p_z(self.v_0)
    ...
    def elbo(self, x, x_recon, z, mean, log_var, z_T, v_0, v_T):

        # [P, B]
        temp_elbo = self._log_likelihood(x, x_recon) + self._log_p_z(z_T) - self._log_q_z_given_x(z, mean, log_var) + (self.T*tf.log(self.friction)) + self._log_p_z(v_T) - self._log_p_z(v_0)
