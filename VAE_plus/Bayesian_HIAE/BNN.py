...
class BNN(object):
    ...
    def feedforward(self, W_list, x):
        ...
        cur_val = x
        #[B,P,X]->[B,P,1,X]
        cur_val = tf.reshape(cur_val, [self.batch_size, P, 1, self.input_size])
        ...
        for layer_i in range(len(self.net)-1):
            ...
            #Concat 1 to input for biases  [B,P,1,X]->[B,P,1,X+1]
            cur_val = tf.concat([cur_val,tf.ones([self.batch_size,P,1, 1])], axis=3)
            #[X,X']->[1,1,X,X']
            W = tf.reshape(W, [1, 1, input_size_i, output_size_i])
            W = tf.tile(W, [self.batch_size, P, 1,1])

            #Forward Propagate  [B,P,1,X]*[1,1,X,X']->[B,P,1,X']
