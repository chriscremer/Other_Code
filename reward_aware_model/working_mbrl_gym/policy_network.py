...
class Policy():
    ...
    def J_equation(self):
        ...
        def fn_over_timesteps(prev_reward_and_z, current_scan_over):
            ...
            new_obs = tf.pack(particles_obs, axis=1) #[B,P,Z]
            new_obs = tf.reshape(new_obs, [self.batch_size, self.n_particles*self.input_size]) #[B,PZ]
            

            output = tf.concat(1, [avg_particle_reward, new_particles])
            output = tf.concat(1, [output, new_obs])
        ...
        x_init = tf.zeros([self.batch_size, self.input_size*self.n_particles]) #used for debugging
        initialization = tf.concat(1, [reward_init, z_init])
        initialization = tf.concat(1, [initialization, x_init])
        ...
        # self.get_r = rewards_and_particles
        ...
        # [T,B,1]
        rewards = tf.slice(rewards_and_particles, [0,0,0], [self.n_timesteps, self.batch_size, 1])
        # print rewards
        # self.get_r = rewards

        # self.obs_for_train = tf.slice(rewards_and_particles, [0,0,1+self.n_particles*self.z_size], [self.n_timesteps, self.batch_size, self.input_size])
        # print self.obs_for_train
