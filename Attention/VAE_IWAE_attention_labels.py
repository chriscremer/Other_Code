...
class VAE(object):
    ...
    def __init__(self, batch_size):
        ...
        self.transfer_fct = tf.tanh #tf.nn.softplus #
    ...
    def _log_likelihood(self, t, pred_no_sig):
        ...
        attention = tf.nn.softmax(tf.reshape(attention, [1,self.batch_size,784]))

        reconstr_loss = reconstr_loss * attention
        reconstr_loss = 784* tf.reduce_sum(reconstr_loss, 2) #sum over dimensions
        ...
        for epoch in range(epochs):
            ...
            for step in range(n_datapoints/self.batch_size):
                ...
                if step % display_step == 0:
                    ...
                    cost = self.sess.run((self.elbo), feed_dict={self.x: batch, self.y: batch_y})
                    cost = -cost #because I want to see the NLL

                    print "Epoch", str(epoch)+'/'+str(epochs), 'Step:%04d' % (step) +'/'+ str(n_datapoints/self.batch_size), "cost=", "{:.6f}".format(float(cost))#, 'time', time.time() - start
