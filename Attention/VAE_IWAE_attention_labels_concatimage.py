...
class VAE(object):
        ...
        for epoch in range(epochs):
            ...
            for step in range(n_datapoints/self.batch_size):
                ...
                if step % display_step == 0:
                    ...
                    elbo, cost, px, py, pz, qz = self.sess.run((self.elbo, self.cost, self.px, self.py, self.pz, self.qz), feed_dict={self.x: batch, self.y: batch_y})
    ...
    def reconstruct(self, sampling, data, labels):
        ...
        while len(batch) != self.batch_size:
            ...
            img2 = np.reshape(data[ind2], [28,28])
