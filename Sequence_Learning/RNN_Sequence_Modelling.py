...
#DATA
######################################################################
def make_ball_gif(n_frames=3, f_height=5, f_width=6, ball_size=2):
...
class RNN():
	...
	def __init__(self, batch_size=1):
		...
		self.filter_width = 2
		self.filter_out_channels1 = 5

		self.flatten_len1 = 30
		self.fc1_output_len = 20
		...
		# # self.path_to_load_variables = home+'/storage/viewnyx/model_variables/v2_rnn_vars_pro_3.ckpt' #rao
		self.path_to_load_variables = ''
		self.path_to_save_variables = home+'/Documents/tmp/rnn_sequence.ckpt'
		...
		with self.graph.as_default():
			...
			self.target = tf.placeholder("float", shape=[self.batch_size, self.n_time_steps, self.image_height, self.image_width, self.n_channels])
			...
			self.pred_update_vars = tf.Variable(tf.truncated_normal([self.fc1_output_len+self.state_size, self.pred_hidden_layer],stddev=0.1))
			...
			self.weight_decay = self.lmbd * (tf.nn.l2_loss(self.fc1_weights) +
											...
											tf.nn.l2_loss(self.forget_gate_variables) +
											tf.nn.l2_loss(self.update_gate_variables) +
											tf.nn.l2_loss(self.pred_update_vars) +
											tf.nn.l2_loss(self.pred_update_vars2) +
											tf.nn.l2_loss(self.forget_gate_variables_b) +
											tf.nn.l2_loss(self.update_gate_variables_b) +
											tf.nn.l2_loss(self.pred_update_vars_b) +
											tf.nn.l2_loss(self.pred_update_vars2_b) + 
											tf.nn.l2_loss(self.forget_gate_variables2) +
											tf.nn.l2_loss(self.forget_gate_variables2_b) +
											tf.nn.l2_loss(self.update_gate_variables2) +
											tf.nn.l2_loss(self.update_gate_variables2_b))
	...
	def state_update(self, conv_image, state):
		...
		update = tf.nn.relu(update)
		forget = 1. - forget
		state = state + (forget * update)

		return state


	def prediction_update(self, conv_image, state):

		out = tf.matmul(tf.concat(1, [conv_image, state]), self.pred_update_vars) + self.pred_update_vars_b
		out = tf.nn.relu(out)
	...
	def feedforward(self, sample):
		...
		for time_step in range(self.n_time_steps):
			...
			conv_output = self.conv_feedforward(tf.reshape(tf.slice(sample, [0,time_step,0,0,0], [self.batch_size, 1,self.image_height,self.image_width,1]), [self.batch_size,self.image_height,self.image_width,1]))
			...
			pred = tf.reshape(pred, [self.image_height,self.image_width,1])
