...
#this works if theres only on speed, if not , theres uncertainty
...
#DATA
######################################################################
def make_ball_gif(n_frames=2, f_height=6, f_width=6, ball_size=2):
...
class Predict_Next_Frame():
	...
	def __init__(self, batch_size=1):
		...
		with self.graph.as_default():
			...
			#PLACEHOLDERS
			self.input = tf.placeholder("float", shape=[self.batch_size,  self.image_height, self.image_width, self.n_channels])
			...
			self.pred_update_vars = tf.Variable(tf.truncated_normal([self.fc1_output_len, self.pred_hidden_layer],stddev=0.1))
