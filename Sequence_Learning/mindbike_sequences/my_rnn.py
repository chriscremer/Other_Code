...
# so I want to make this so that it takes ..
        #### The above could be ignored initially
            # so just start with last given frame and 0 state
            # it wont know the velocity/momemtum, so I dont like it
            # so NO, I shoudldnt just update state
            # it should try to predict the next frame everytime
            # but in this loop i always update the prediction with the actual frame
            # so its jsut one step prediction but the state is updated and it doenst get screwed by bad predictions
            # the next scan will be the same but wont be fixed with real frames
...
class RNN():
    ...
    def __init__(self, input_size, action_size, state_size, output_size, max_sequence_length, batch_size=5):
        ...
        # Graph Input 
        self.frames = tf.placeholder(tf.float32, [None, self.input_size])  #[t,X]
        self.actions_with_frames = tf.placeholder(tf.float32, [None, self.action_size]) # [t,A]
        self.actions_without_frames = tf.placeholder(tf.float32, [None, self.action_size]) # [T-t, A]
        self.n_frames = tf.placeholder(tf.int32, []) #t
        self.all_frames = tf.placeholder(tf.float32, [None, self.input_size]) #[T,X]
    ...
    def _initialize_weights(self):
        ...
        self.update_gate_variables = tf.Variable(xavier_init(self.action_size+self.state_size+self.input_size, self.state_size))
        self.update_gate_variables_b = tf.Variable(tf.zeros([self.state_size]))
    ...
    def l2_regularization(self):
        ...
        vars_ = [self.forget_gate_variables, self.forget_gate_variables_b,
    ...
    def update_state(self, action, prev_frame, prev_state):
        ...
        #Concat all inputs
        inputs = tf.concat(0, [tf.concat(0, [action, prev_frame]), prev_state])
    ...
    def predict_frame(self, prev_frame, state, current_action):
        ...
        #Concat all inputs
        inputs = tf.concat(0, [tf.concat(0, [state, prev_frame]), current_action])
        inputs = tf.reshape(inputs, [1,-1])
    ...
    def fn(self, previous_output, current_input):
        ...
        prev_state = tf.slice(previous_output, [0], [self.state_size])
        prev_frame = tf.slice(previous_output, [self.state_size], [self.input_size])
        prev_frame_pred = tf.slice(previous_output, [self.state_size+self.input_size], [self.input_size])

        #Unpack input (frame, action)
        frame = tf.slice(current_input, [0], [self.input_size])
        action = tf.slice(current_input, [self.input_size], [self.action_size])
        ...
        #Pack results
        pack_output = tf.concat(0, [tf.concat(0, [state, frame]), frame_t_pred])
    ...
    def fn2(self, previous_output, current_input):
        ...
        action = tf.slice(current_input, [0], [self.action_size])
        ...
        #Pack results
        pack_output = tf.concat(0, [state, frame_t_pred])
    ...
    def feedforward(self, frames, actions_with_frames, actions_without_frames):
        ...
        # frames: [t, X]
        # actions_with_frames: [t, A]
        # actions_without_frames: [T-t, A]

        '''

        # print frames
        # print actions_with_frames

        #Concat frames with actions [t, X+A]
        frames_actions = tf.concat(1, [frames, actions_with_frames])

        init_state = tf.zeros([self.state_size])
        init_prev_frame = tf.zeros([self.input_size])
        init_frame_pred = tf.zeros([self.input_size])
        initializer = tf.concat(0, [tf.concat(0, [init_state, init_prev_frame]), init_frame_pred])
        ...
        # last_output = tf.gather(outputs, self.n_frames)
        # print outputs
        last_output = tf.slice(outputs, [self.n_frames-1,0], [1,self.state_size+self.input_size+self.input_size])
        last_output = tf.reshape(last_output, [-1])
        # print last_output
        prev_state = tf.slice(last_output, [0], [self.state_size])
        prev_frame = tf.slice(last_output, [self.state_size], [self.input_size])
        initializer = tf.concat(0, [prev_state, prev_frame])
        ...
        outputs = tf.slice(outputs, [0,self.state_size+self.input_size], [self.n_frames, self.input_size])
        outputs2 = tf.slice(outputs2, [0,self.state_size], [self.max_sequence_length-self.n_frames, self.input_size])
    ...
    def train(self, frames, actions, steps=1000, display_step=10, path_to_load_variables='', path_to_save_variables=''):
        ...
        for step in range(steps):
            ...
            _ = self.sess.run(self.optimizer, feed_dict={self.frames: frames_, 
                                                    self.actions_with_frames: actions_with_frames, 
                                                    self.actions_without_frames: actions_without_frames,
            ...
            if step % display_step == 0:
                ...
                reconst_loss = self.sess.run(self.reconst_loss, feed_dict={self.frames: frames_, 
                                                    self.actions_with_frames: actions_with_frames, 
                                                    self.actions_without_frames: actions_without_frames,
                                                    self.n_frames: timestep,
                                                    self.all_frames: all_frames})
            ...
            #     seq = self.sess.run(tf.sigmoid(self.prediction), feed_dict={self.frames: frames_, 
            #                                         self.actions_with_frames: actions_with_frames, 
            #                                         self.actions_without_frames: actions_without_frames,
            #                                         self.n_frames: timestep,
            #                                         self.all_frames: all_frames})
    ...
    def test(self, frames, actions, path_to_load_variables=''):
        ...
        # Save a sequence
        seq = self.sess.run(tf.sigmoid(self.predictions), feed_dict={self.frames: frames_, 
                                            self.actions_with_frames: actions_with_frames, 
                                            self.actions_without_frames: actions_without_frames,
                                            self.n_frames: timestep,
                                            self.all_frames: all_frames})
