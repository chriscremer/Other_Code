...
#so in training set, 0-74 will be from meta and 75-149 will be other
#in test set, 0-71 will be meta and 72-143 will be other
...
with open(data_other, 'rU') as f:
		...
		for row in reader:
			if row[0] in sig_genes_names:
				...
				for expr in row:
					if c in set_1:
						X[c-1+len(set_1)].append(float(expr))
					c += 1
			...
			if row[0] in sig_genes_names:
				...
				for expr in row:
					if c in set_2:
						...
						X[c-(len(set_1)+1)].append(float(expr))
		...
		for row in reader:
			if row[0] in sig_genes_names:
				...
				for expr in row:
					if c in set_2:
						print c
						X[c- (len(set_1) - len(set_2) + 1)].append(float(expr))
...
plt.xlim(1,1.07)
plt.ylim(-0.02,1.03)
plt.title("Logistic Regression Metastasis")
plt.ylabel("Scale")
...
plt.savefig('log_reg_metastasis.pdf')
