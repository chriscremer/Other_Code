...
class QuadraticCost:
    ...
    def cost(a, y):
        ...
        WHY linalg.norm?? what is it doing?
        """
...
class Network():
    ...
    def feedforward(self, a):
        ...
        """
        last_activation = self.num_layers - 1
        weight_count = 1
        for b, w in zip(self.biases, self.weights):
            if weight_count == last_activation:
                a = np.dot(w, a)+b
            else:
    ...
    def backprop(self, x, y):
        ...
        #since a of last layer is z, the derivative of a wrt z is 1
        #delta is c' * a' 
...
#load data
data_file = '/data1/morrislab/ccremer/TCGA_data/find_metastatic/test2/output_other_exps.txt'



#EXAMPLE
'''
x1 = np.reshape(np.array([2]), (1,1))
y1 = np.reshape(np.array([0.1]), (1,1))
x2 = np.reshape(np.array([3]), (1,1))
y2 = np.reshape(np.array([0.2]), (1,1))
training_data = [(x1,y1), (x2,y2)]
print training_data
print
net = Network([1, 6, 1])
evaluation_cost, evaluation_accuracy, training_cost, training_accuracy = net.SGD(training_data, 1000, 1, 0.9, monitor_training_cost=True)
'''
