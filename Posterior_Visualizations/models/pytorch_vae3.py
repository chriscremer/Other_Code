...
if __name__ == "__main__":
    ...
    if eval_IW:
        print('\nTesting with IW, Train set')
        model.test(data_x=train_x[:10000], batch_size=20, display=100, k=200)

        print('\nTesting with IW, Test set')
        model.test(data_x=test_x, batch_size=20, display=100, k=200)

    if eval_AIS:
        print('\nTesting with AIS, Train set')
        test_ais(model, data_x=train_x[:10000], batch_size=100, display=10, k=10, n_intermediate_dists=50)

        print('\nTesting with AIS, Test set')
        test_ais(model, data_x=test_x, batch_size=100, display=10, k=10, n_intermediate_dists=50)
