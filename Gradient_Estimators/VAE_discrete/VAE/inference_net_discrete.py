...
from enc_dec_discrete import Encoder, Decoder
...
class Inference_Q_Bernoulli(nn.Module):
    def __init__(self, kwargs):
        super(Inference_Q_Bernoulli, self).__init__()
        ...
        # q(z|x)
        self.inference_net = Encoder(kwargs=kwargs, input_nc=3, image_encoding_size=self.x_enc_size, 
                                    n_residual_blocks=self.enc_res_blocks, input_size=self.input_size)

        self.optimizer_x = optim.Adam(self.inference_net.parameters(), lr=lr, weight_decay=.0000001)
    ...
    #     logits = self.image_encoder2(x)
    #     # print (mean_logvar.shape)
    #     # fdsafda
    #     # mean = mean_logvar[:,:6]
