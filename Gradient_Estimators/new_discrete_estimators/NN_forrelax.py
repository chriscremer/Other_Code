...
class NN(nn.Module):
    ...
    def train(self, func, dist, save_dir, bern_param, early_stop=100):
        ...
        while not done:
            ...
            # samps = torch.log(bern_param/(1-bern_param)) + torch.log(v/(1-v))
            # z= torch.sigmoid(samps)
            # # samps = H(samps)
            ...
            # print (samps.size())
            obj_value = func(bs)#.unsqueeze(1) #[B,1]
            pred = self.net(zs) #[B,1]
            ...
            if len(last_100) <  200:
                ...
                if first or (last_100_avg< best_100).data.numpy():
                    ...
                    if not_better_counter == early_stop:
                        break
                last_100 = []
