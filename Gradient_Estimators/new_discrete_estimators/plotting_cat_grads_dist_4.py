...
# going to plot reinforce, reinforce critic, relax, simplax
...
# rewards = torch.tensor([5., 10., 20.]) #* 100.
...
if train_:
    ...
    for i in range(n_steps+1):
        ...
        # loss = torch.mean(((reward.view(B,1) - cz)  )**2)
        # loss = torch.mean((grad)**2)

        # print (gradlogprob, gradcz)
        # fasdfs
        ...
        # if i % (n_steps/10)==0:
        if i % (100)==0:
            print(i, 'loss:', loss.data.numpy(), ' dif:', torch.mean(torch.abs(reward.view(B,1) - cz)).data.numpy(), 'pred:', cz[0].data.numpy(), 'R:', reward[0].data.numpy(), )#'  z:', z[0].data.numpy(), '  b:', b[0].data.numpy(), '  z_t:', z_tilde[0].data.numpy(),) # '  pred2:', cz_tilde_after[0].data.numpy(),)
...
# print (torch.mean((grads[:,0] > -.5).float()))
# fsaas
...
for i in range(500):
    ...
    gradlogprob = torch.autograd.grad(outputs=logprob, inputs=(logits), retain_graph=True)[0]
    c = critic.net(logits)
    loss = torch.mean( ((reward - c)*gradlogprob)**2 )
...
for i in range (N):
    ...
    gradlogprob = torch.autograd.grad(outputs=logprob, inputs=(logits), retain_graph=True)[0]
    c = critic.net(logits)
    grads_reinforcewithcritic.append( (reward-c) *gradlogprob)
    
print ()
grads_reinforcewithcritic = torch.stack(grads_reinforcewithcritic).view(N,C)
# print (grads.shape)
grad_mean_reinforce_withcritic = torch.mean(grads_reinforcewithcritic,dim=0)
grad_std_reinforce_withcritic = torch.std(grads_reinforcewithcritic,dim=0)

print ('REINFORCE with critic')
print ('mean:', grad_mean_reinforce_withcritic)
print ('std:', grad_std_reinforce_withcritic)
print ()
# print ('True')
# print ('[-.5478, .1122, .4422]')
# print ('dif:', np.abs(grad_mean_reinforce.numpy() -  true))
...
if train_:
    ...
    for i in range(n_steps+1):
        ...
        # print(reward.shape, cz_tilde.shape, logprob.shape, cz.shape)
        # fasdfad

        # estimator = (reward - cz_tilde) * logprob + cz - cz_tilde
        # grad = torch.autograd.grad([torch.mean(estimator)], [logits], create_graph=True, retain_graph=True)[0]
        # loss = torch.mean((grad)**2)
...
ax.set_title('R={}, p={:.2f}'.format(to_print2(rewards[0]), to_print2(probs[0][0]))) 
ax.text(1,2, 'mean:{:.3f}\nstd:{:.3f}'.format(to_print2(grad_mean_reinforce[0]), to_print2(grad_std_reinforce[0])))  
...
ax.set_xlim(x_lim_left, x_lim_right)
ax.set_ylim(0., y_lim_top)
ax.set_title('R={}, p={:.2f}'.format(to_print2(rewards[1]), to_print2(probs[0][0]))) 
ax.text(1,2, 'mean:{:.3f}\nstd:{:.3f}'.format(to_print2(grad_mean_reinforce[1]), to_print2(grad_std_reinforce[1]))) 
...
ax.set_xlim(x_lim_left, x_lim_right)
ax.set_ylim(0., y_lim_top)
ax.set_title('R={}, p={:.2f}'.format(to_print2(rewards[2]), to_print2(probs[0][0]))) 
ax.text(1,2, 'mean:{:.3f}\nstd:{:.3f}'.format(to_print2(grad_mean_reinforce[2]), to_print2(grad_std_reinforce[2]))) 
...
col =0
ax = plt.subplot2grid((rows,cols), (row,col), frameon=False, colspan=1, rowspan=1)
ax.hist(to_print2(grads_reinforcewithcritic[:,0]), bins=n_bins, density=True)
ax.plot([grad_mean_reinforce_withcritic[0],grad_mean_reinforce_withcritic[0]], [0,y_lim_top])
ax.set_xlim(x_lim_left, x_lim_right)
ax.set_ylim(0., y_lim_top)
# ax.set_xlabel('x0')
ax.set_ylabel('reinforce with critic')
ax.text(1,2, 'mean:{:.3f}\nstd:{:.3f}'.format(to_print2(grad_mean_reinforce_withcritic[0]), to_print2(grad_std_reinforce_withcritic[0]))) 

col =1
# row =1
ax = plt.subplot2grid((rows,cols), (row,col), frameon=False, colspan=1, rowspan=1)
ax.hist(to_print2(grads_reinforcewithcritic[:,1]), bins=n_bins, density=True)
ax.plot([grad_mean_reinforce_withcritic[1],grad_mean_reinforce_withcritic[1]], [0,y_lim_top])
ax.set_xlim(x_lim_left, x_lim_right)
ax.set_ylim(0., y_lim_top)
# ax.set_xlabel('x1')
ax.text(1,2, 'mean:{:.3f}\nstd:{:.3f}'.format(to_print2(grad_mean_reinforce_withcritic[1]), to_print2(grad_std_reinforce_withcritic[1])))  


col =2
# row =1
ax = plt.subplot2grid((rows,cols), (row,col), frameon=False, colspan=1, rowspan=1)
ax.hist(to_print2(grads_reinforcewithcritic[:,2]), bins=n_bins, density=True)
ax.plot([grad_mean_reinforce_withcritic[2],grad_mean_reinforce_withcritic[2]], [0,y_lim_top])
ax.set_xlim(x_lim_left, x_lim_right)
ax.set_ylim(0., y_lim_top)
ax.text(1,2, 'mean:{:.3f}\nstd:{:.3f}'.format(to_print2(grad_mean_reinforce_withcritic[2]), to_print2(grad_std_reinforce_withcritic[2]))) 
...
# ax.set_xlabel('x0')
ax.set_ylabel('relax')
ax.text(1,2, 'mean:{:.3f}\nstd:{:.3f}'.format(to_print2(grad_mean_relax[0]), to_print2(grad_std_relax[0]))) 
...
ax.set_ylim(0., y_lim_top)
# ax.set_xlabel('x1')
ax.text(1,2, 'mean:{:.3f}\nstd:{:.3f}'.format(to_print2(grad_mean_relax[1]), to_print2(grad_std_relax[1])))   
...
ax.set_ylim(0., y_lim_top)
# ax.set_xlabel('x2')
ax.text(1,2, 'mean:{:.3f}\nstd:{:.3f}'.format(to_print2(grad_mean_relax[2]), to_print2(grad_std_relax[2])))    
...
col =0
ax = plt.subplot2grid((rows,cols), (row,col), frameon=False, colspan=1, rowspan=1)
ax.hist(to_print2(grads_simplax[:,0]), bins=n_bins, density=True)
ax.plot([grad_mean_simplax[0],grad_mean_simplax[0]], [0,y_lim_top])
ax.set_xlim(x_lim_left, x_lim_right)
ax.set_ylim(0., y_lim_top)
# ax.set_xlabel('x0')
ax.set_ylabel('simplax')
ax.text(1,2, 'mean:{:.3f}\nstd:{:.3f}'.format(to_print2(grad_mean_simplax[0]), to_print2(grad_std_simplax[0]))) 

col =1
# row =1
ax = plt.subplot2grid((rows,cols), (row,col), frameon=False, colspan=1, rowspan=1)
ax.hist(to_print2(grads_simplax[:,1]), bins=n_bins, density=True)
ax.plot([grad_mean_simplax[1],grad_mean_simplax[1]], [0,y_lim_top])
ax.set_xlim(x_lim_left, x_lim_right)
ax.set_ylim(0., y_lim_top)
# ax.set_xlabel('x1')
ax.text(1,2, 'mean:{:.3f}\nstd:{:.3f}'.format(to_print2(grad_mean_simplax[1]), to_print2(grad_std_simplax[1])))   


col =2
# row =1
ax = plt.subplot2grid((rows,cols), (row,col), frameon=False, colspan=1, rowspan=1)
ax.hist(to_print2(grads_simplax[:,2]), bins=n_bins, density=True)
ax.plot([grad_mean_simplax[2],grad_mean_simplax[2]], [0,y_lim_top])
ax.set_xlim(x_lim_left, x_lim_right)
ax.set_ylim(0., y_lim_top)
# ax.set_xlabel('x2')
ax.text(1,2, 'mean:{:.3f}\nstd:{:.3f}'.format(to_print2(grad_mean_simplax[2]), to_print2(grad_std_simplax[2])))    
