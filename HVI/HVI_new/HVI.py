...
class HVI(object):
    ...
    def __init__(self, hyperparams):
        ...
        self.n_transitions = 5 #this is leapfrog steps, whereas T=1 like the paper
        ...
        # q(z|x)
        net1 = NN([self.x_size, 300, self.z_size*2], tf.nn.elu, self.batch_size)
        # q(v|x,z) 
        net2 = NN([self.x_size+self.z_size, 300, self.z_size*2], tf.nn.elu, self.batch_size)
        # r(v|x,z)
        net2 = NN([self.x_size+self.z_size, 300, self.z_size*2], tf.nn.elu, self.batch_size)
        # p(x|z)
        net2 = NN([self.x_size+self.z_size, 300, self.z_size*2], tf.tanh, self.batch_size)
    ...
    def leapfrogs(self, z, v, decoder, x):
        '''
        z0: [B,Z]
        '''
    ...
    def log_probs(self, x, q_zlx, q_vlxz, r_vlxz, p_xlz):
        ...
        v0_mean, v0_logvar = split_mean_logvar(q_vlxz.feedforward(xz)) #[B,Z]
        v0 = sample_Gaussian(v0_mean, v0_logvar, self.n_z_particles) #[P,B,Z]
        log_qv0 = log_norm(v0, v0_mean, v0_logvar) #[P,B]

        # Transform [P,B,Z]
        zT, vT = leapfrogs(z0,v0,p_xlz,x)

        # Reverse model
        xz = tf.concat([x,zT], axis=1) #[B,X+Z]
        vt_mean, vt_logvar = self.split_mean_logvar(r_vlxz.feedforward(xz)) #[B,Z]
        log_rv = log_norm(vT, vt_mean, vt_logvar) #[B]
    ...
    def train(self, train_x, valid_x=[], display_step=5, 
                ...
                epochs=10, batch_size=20, n_z_particles=3):
        '''
        Train.
        '''
...
if __name__ == '__main__':
    ...
    model.train(train_x=train_x,
                epochs=2, batch_size=20, n_z_particles=1, display_step=1000,
