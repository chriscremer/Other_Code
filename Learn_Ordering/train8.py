...
for step_not_including_load in range(max_steps+1):
    ...
    for i_order in range(n_steps):
        ...
        mean, logsd = model(input_)
        logsd = torch.clamp(logsd, min=-7.)
        # logsd = torch.ones_like(logsd) * .1

        # alpha = torch.exp(mean)
        # beta = torch.exp(logsd)

        # alpha = torch.clamp(alpha, min=1e-3, max=1000.)
        # beta = torch.clamp(beta, min=1e-3, max=1000.)

        # alpha = torch.clamp(alpha, min=1., max=500.)
        # beta = torch.clamp(beta, min=1., max=500.)


        # alpha = torch.sigmoid(mean * sig_val) * sig_val +.01
        # beta = torch.sigmoid(logsd * sig_val) * sig_val +.01

        # alpha = torch.tanh(mean) * 5.  + 5.1
        # beta = torch.tanh(logsd) * 5.  + 5.1


        # dist = torch.distributions.beta.Beta(concentration1=alpha, concentration0=beta)

        # logprob = dist.log_prob(img) #, mean, logsd)
        ...
        if (logprob!=logprob).any():
            print ('step', step, '  -nans in logprob!!')
            # print (nobj_opt)
        ...
        # img_logsd_preds_vals.append( torch.mean(torch.log(logsd)).detach())
    ...
    if step %args.print_every == 0:
        ...
        myprint( (step, 'T:{:.1f}'.format(T), 
                ...
                # 'b:{:.2f}'.format(numpy(torch.min(beta))),
                # 'b:{:.2f}'.format(numpy(torch.mean(beta))),
                # 'b:{:.2f}'.format(numpy(torch.max(beta))),
                ))
