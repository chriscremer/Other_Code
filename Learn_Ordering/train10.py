...
def make_contour_subplot(rows, cols, row, col, image, text, legend=False):
    ...
    # print (image.shape)
    image = image.view(image_dim,image_dim) 
...
def make_image_subplot(rows, cols, row, col, image, text):
    ...
    if image.shape[0] != 1:
        ...
        image = image.view(image_dim,image_dim) * 255.
        image = image.data.cpu().numpy() 
        image = np.uint8(image)
...
class Mixture_Dist():
    ...
    def logprob(self, x):
        ...
        # print (lme.shape)
        lme = lme.view(B,3,self.dim,self.dim)
        # fadsf
    ...
    def sample(self):
        ...
        samp = samp.view(B,3,self.dim,self.dim)
    ...
    def get_mean(self):
        ...
        mean = mean.view(B,3,self.dim,self.dim)
        return mean
    ...
    def get_var(self):
        ...
        var = var.view(B,3,self.dim,self.dim)
...
def make_sample_plot(model):
    ...
    masked_input = torch.zeros(1,3,image_dim,image_dim).cuda()
    allseenpixels_mask = torch.zeros(B,1,image_dim*image_dim).cuda()
    ...
    for i_order in range(n_steps_sample):
        ...
        means, logsds, mixture_weights, mean, logsd, NLL_pixelavg, logsd_pixelavg, logprob = make_preds(masked_input, masked_input)

        dist = Mixture_Dist(k=args.n_mixtures, means=means, logsds=logsds, mixture_weights=mixture_weights)
        ...
        if i_order != n_steps_sample-1:
            ...
            NLL_pred_scaled = NLL_pred_scaled * (1.-allseenpixels_mask.view(B,image_dim,image_dim))  #remove the pixels that are revealed
            ...
            # print (torch.sum(sampled_pixels_mask))
            



            samp = dist.sample()
            samp = torch.clamp(samp, min=1e-5, max=1-1e-5)

            # print (samp.shape)
            # fdsafds


            new_pixels = samp * sampled_pixels_mask.view(B,1,image_dim,image_dim)

            # Accumulate mask and mask the image
            allseenpixels_mask = allseenpixels_mask.view(B,image_dim*image_dim) + sampled_pixels_mask
            # input_ = (img * allseenpixels_mask.view(B,1,112,112)).detach()
            masked_input = masked_input + new_pixels

            if i_order % 100 == 0:
                print (i_order)
                # samp_images.append(torch.clamp(masked_input, min=1e-5, max=1-1e-5))

            if i_order <40   and i_order%4==0 :
                samp_images.append(masked_input)
                samp_images_mean.append(torch.clamp(mean, min=1e-5, max=1-1e-5))
                samp_images_logsd.append(logsd_pixelavg)
                samp_images_samp.append(samp)

                

            # print(logsd_pixelavg.shape)
            # fdsf
            ...
            new_pixels = samp * (1.-allseenpixels_mask.view(B,1,image_dim,image_dim))

            sampled_image = masked_input + new_pixels
    ...
    # cols = min(3,n_steps)
    cols = len(samp_images) #/ 10 #1 #n_steps_sample
    rows = 4
    ...
    for ii in range(cols):
        ...
        make_image_subplot(rows=rows, cols=cols, row=1, col=ii, image=samp_images_mean[ii][img_idx], text='Mean')
        make_contour_subplot(rows=rows, cols=cols, row=2, col=ii, image=samp_images_logsd[ii][img_idx], text='')
        make_image_subplot(rows=rows, cols=cols, row=3, col=ii, image=samp_images_samp[ii][img_idx], text='')
...
def sample_pixels(logsd_pixelavg, k, allseenpixels_mask):
    ...
    # Get top error pixels - ie defining sampling order
    NLL_pred_scaled = logsd_pixelavg - torch.min(logsd_pixelavg) +.001 #shift so 0 is lowest
    NLL_pred_scaled = (NLL_pred_scaled * (1.-allseenpixels_mask.view(B,image_dim,image_dim))).view(B,-1)  #remove the pixels that are revealed

    n_k = min(k*5,image_dim*image_dim)
    values, indices = torch.topk(NLL_pred_scaled, k=n_k, dim=1, largest=True, sorted=True) 

    # Take random set of k
    idx = torch.randperm(n_k) 
    indices = indices[:,idx]
    indices = indices[:,:k]
...
def compute_LLs(img):
    ...
    masked_input = torch.zeros_like(img).cuda()
    allseenpixels_mask = torch.zeros(B,1,image_dim*image_dim).cuda()
    ...
    k = 1
    n_steps = (image_dim* image_dim) -2 #5000

    NLLs = []
    prev_NLL=0

    for i_order in range(n_steps):
        if i_order % 100==0 and i_order!=0:
            print (i_order, '{:.3f}'.format(numpy(-torch.mean(logprob))), '{:.4f}'.format(dif))

            

        means, logsds, mixture_weights, _, _, _, _, _ = make_preds(masked_input, img)
        # means, logsds, mixture_weight = model(input_)
        # means = torch.clamp(means, min=0., max=1.)
        # logsds = torch.clamp(logsds, min=-4.5, max=1.)

        #Update
        allseenpixels_mask = allseenpixels_mask.view(B,1,image_dim,image_dim)
        ...
        current_mw = current_mw* allseenpixels_mask + mixture_weights* (1-allseenpixels_mask)

        # NEED THIS BECAUSE WANT LOGPROB GIVEN PREVIOUS PREDS
        dist = Mixture_Dist(k=args.n_mixtures, means=current_mean, logsds=current_logsd, mixture_weights=current_mw)
        ...
        allseenpixels_mask = allseenpixels_mask.view(B,image_dim*image_dim) + sampled_pixels_mask # Accumulate mask 
        masked_input = (img * allseenpixels_mask.view(B,1,image_dim,image_dim)).detach()

        # print (logprob.shape)
        temp = -torch.mean(logprob)
        # print ()
        # print (temp)
        temp = numpy(temp)
        # print (temp)
        ...
        dif = prev_NLL - numpy(-torch.mean(logprob))
        prev_NLL = numpy(-torch.mean(logprob))
...
def make_preds(masked_image, true_image):

    means, logsds, mixture_weights = model(masked_image)
    # means = torch.clamp(means, min=0., max=1.)
    ...
    dist = Mixture_Dist(k=args.n_mixtures, means=means, logsds=logsds, mixture_weights=mixture_weights)
    logprob = dist.logprob(true_image)
    ...
    return means, logsds, mixture_weights, mean, logsd, NLL_pixelavg, logsd_pixelavg, logprob
...
model = NN(in_channels=3, hidden_channels=64, channels_out=3, k=args.n_mixtures).cuda()
...
for step_not_including_load in range(max_steps+1):
    ...
    masked_input = torch.zeros_like(img, requires_grad=True).cuda()
    allseenpixels_mask = torch.zeros(B,1,image_dim*image_dim).cuda()
    total_cost = 0
    total_NLL = 0
    ...
    if (step % args.plotimages_every == 0 or step %args.print_every == 0 ):
        ...
        k = np.random.randint(low=1, high=image_dim*image_dim - 2)
    ...
    for i_order in range(n_steps):
        ...
        if i_order != n_steps-1:
            ...
            with torch.no_grad():
                means, logsds, mixture_weights, mean, logsd, NLL_pixelavg, logsd_pixelavg, logprob = make_preds(masked_input, img)
                sampled_pixels_mask = sample_pixels(logsd_pixelavg, k, allseenpixels_mask)
                img_mean_pred_masked = (torch.clamp(mean, min=1e-5, max=1-1e-5) * (1-allseenpixels_mask.view(B,1,image_dim,image_dim))).detach()
                allseenpixels_mask = allseenpixels_mask.view(B,image_dim*image_dim) + sampled_pixels_mask # Accumulate mask 
                masked_input = (img * allseenpixels_mask.view(B,1,image_dim,image_dim)).detach()
        ...
        true_error.append(NLL_pixelavg.detach().view(B,1,image_dim,image_dim))
    ...
    if step % args.plotimages_every == 0 and step!=0:
        ...
        if image_dim == 112:
            grad = torch.autograd.grad(outputs=mean[0][0][55][55], inputs=masked_input)[0]
        if image_dim == 32:
            grad = torch.autograd.grad(outputs=mean[0][0][15][15], inputs=masked_input)[0]
        ...
        with torch.no_grad():
            ...
            make_contour_subplot(rows=rows, cols=cols, row=0, col=2, image=grad[0].view(1,image_dim,image_dim), text='Grad')
            ...
            for ii in range(cols):
                ...
                make_image_subplot(rows=rows, cols=cols, row=2, col=ii, image=img_preds_masked[ii][img_idx], text='Img Mean Pred Masked')
                ...
                make_image_subplot(rows=rows, cols=cols, row=5, col=ii, image=img_preds[ii][img_idx], text='Img Mean Pred')
