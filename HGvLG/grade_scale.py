...
def getScoreWithTheseSpecificSets(LG_training_set, HG_training_set, LG_test_set, HG_test_set, numbOfGenes):
	...
	if method == 1:
		...
		for gene in sig_dif_abs_sorted[:1000]:
			...
			if LG_aPs_minus_HG_aMs < 0.1 or HG_aPs_minus_LG_aMs < 0.1:
				sig_genes.append(gene[0])

		sig_genes_names = sig_genes[:numbOfGenes]
	...
	for i in range(numbOfGenes):
		parameters.append(1.0/numbOfGenes)
	"""
	# m = numb of samples 
	...
	for i in range(10):
		...
		for parameter in range(len(parameters)):
			...
			derivative = gs_functions.costDerivative(csvFile, parameters, sig_genes_names, LG_training_set, HG_training_set, data, parameter)
	...
	#Score1 = gs_functions.score(LG_training_set, csvFile, LG_gene_avg, HG_gene_avg, sig_genes_names, firstRow)

	#Score2 = gs_functions.score(HG_training_set, csvFile, LG_gene_avg, HG_gene_avg, sig_genes_names, firstRow)
	...
	print '%d genes, accuracy= %f' % (numbOfGenes, accuracy) 
