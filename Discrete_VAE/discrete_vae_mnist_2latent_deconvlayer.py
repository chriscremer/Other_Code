...
# #CONV TRANSPOSE
# filter_ = tf.Variable(tf.truncated_normal([10, 10, 1, 15], stddev=0.1))
# net4 = slim.stack(tf.concat([c_flat,z_flat],axis=1),slim.fully_connected,[375])
# net4 = tf.reshape(net4, [batch_size, 5 , 5 , 15])
# # net4 = tf.nn.local_response_normalization(net4)
# net4 = tf.nn.conv2d_transpose(net4, filter_, [batch_size, 28,28,1] ,strides=[1, 4, 4, 1],padding='VALID')
# logits_x = tf.reshape(net4, [batch_size, -1])


# RESIZE + CONV
# net4 = slim.stack(tf.concat([c_flat,z_flat],axis=1),slim.fully_connected,[100])
# net4 = tf.reshape(net4, [batch_size, 10 , 10 , 1])

# kernel = tf.Variable(tf.truncated_normal([3, 3, 1, 1]))
# net4 = tf.nn.conv2d(net4, kernel, [1, 1, 1, 1], padding='SAME')
# # x = slim.conv2d(x,c,[3,3])
# net4 = tf.image.resize_images(net4,[28,28])

# kernel2 = tf.Variable(tf.truncated_normal([5, 5, 1, 1]))
# net4 = tf.nn.conv2d(net4, kernel2, [1, 1, 1, 1], padding='SAME')
